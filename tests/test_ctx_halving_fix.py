...
class TestContextNotHalvedOnOutputCapError:
    ...
    def test_output_cap_error_sets_ephemeral_not_context_length(self):
        ...
        # The fix: leave enough room for diagnostic context added on retry.
        reserve = min(1024, max(64, available_out // 16))
        agent._ephemeral_max_output_tokens = max(1, available_out - reserve)

        # context_length must be untouched
        assert agent.context_compressor.context_length == old_ctx
        assert agent._ephemeral_max_output_tokens == 18_976
    ...
    def test_output_cap_error_safety_margin(self):
        """The ephemeral value reserves enough room for retry diagnostics."""
        from agent.model_metadata import parse_available_output_tokens_from_error
