...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Use the dispatcher's real cross-platform liveness predicate.

    The old test copy handled Linux zombies but not macOS zombies, so an
    already-exited synthetic worker remained visible to ``kill(pid, 0)`` and
    the test failed even though production ``_pid_alive`` correctly rejected
    it through ``ps -o stat``.  Calling the real predicate keeps this test in
    lockstep with the runtime contract on both platforms.
    """
