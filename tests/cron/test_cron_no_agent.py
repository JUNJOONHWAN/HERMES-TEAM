...
def test_create_job_no_agent_preserves_inference_overrides_as_inactive_metadata(hermes_env):
    from cron.jobs import create_job
    ...
    # Keep the configured axes so dashboards and an eventual switch back to
    # agent mode can round-trip the job without losing operator intent.  The
    # scheduler's no-agent branch never consumes these inactive fields.
    assert job["model"] == "stale/model"
    assert job["provider"] == "custom"
    assert job["base_url"] == "http://localhost:8004/v1"
...
def test_update_job_no_agent_preserves_inference_overrides_as_inactive_metadata(hermes_env):
    ...
    reloaded = get_job(job["id"])
    assert reloaded["no_agent"] is True
    assert reloaded["model"] == "old/model"
    assert reloaded["provider"] == "custom"
    assert reloaded["base_url"] == "http://localhost:8004/v1"
    assert reloaded["model_snapshot"] is None
    assert reloaded["provider_snapshot"] is None
    ...
    assert reloaded["provider"] == "custom"
    assert reloaded["base_url"] == "http://localhost:9999/v1"
    assert reloaded["model_snapshot"] is None
    assert reloaded["provider_snapshot"] is None
