    ...
    *,
    ...
    if sys.platform == "darwin":
        script = (
            ...
            "ps -axo pid,ppid,%cpu,%mem,command -r 2>/dev/null | head -60; "
            "echo '--- process ancestry ---'; "
            f"ps -p {os.getpid()} -o pid=,ppid=,command= 2>/dev/null || true; "
            "echo '--- load average ---'; "
            "sysctl -n vm.loadavg 2>/dev/null || uptime 2>/dev/null || true; "
            "echo '--- recent system log (process exits) ---'; "
            "log show --last 1m --style compact --predicate "
            "'eventMessage CONTAINS[c] \"killed\"' 2>/dev/null | tail -20 || true; "
            "echo '=== end ==='"
        )
    else:
        script = (
    ...
    try:
        ...
        # before it can flush.
        # GNU ``timeout`` is absent on a stock macOS install.  A detached
        # Python helper gives every POSIX target the same bounded execution
        # contract and terminates the diagnostic shell after the deadline.
        runner = (
        ...
        proc = subprocess.Popen(
            [sys.executable, "-c", runner, script, str(timeout_seconds)],
            stdout=fd,
