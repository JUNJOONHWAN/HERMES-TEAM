...
def _build_agent_with_db(db: SessionDB, session_id: str):
    ...
    # This regression is about the session-rotation lock, not auxiliary
    # provider discovery.  Mark the feasibility probe complete so both worker
    # threads reach the lock concurrently and the test remains deterministic
    # in a clean distribution environment with no provider credentials.
