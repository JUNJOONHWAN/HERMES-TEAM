...
def _is_registry_register_call(node: ast.AST) -> bool:
    """Return True for a supported tool-registry ``register`` call expression.

    Most modules import the singleton as ``registry``. Control-plane modules
    also import ``hermes_cli.supervisor_registry`` and therefore use the
    unambiguous ``tool_registry`` alias for the tool singleton. Both spellings
    are part of the built-in discovery contract.
    """
