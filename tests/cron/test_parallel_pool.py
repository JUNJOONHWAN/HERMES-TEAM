...
class TestRunningJobDrain:
    def test_wait_for_running_jobs_blocks_until_job_finishes(self):
        import cron.scheduler as sched

        sched._running_job_ids.clear()
        with sched._running_condition:
            sched._running_job_ids.add("long-market-job")

        def finish():
            time.sleep(0.05)
            with sched._running_condition:
                sched._running_job_ids.discard("long-market-job")
                sched._running_condition.notify_all()

        worker = threading.Thread(target=finish)
        worker.start()
        completed, remaining = sched.wait_for_running_jobs(timeout=1)
        worker.join()

        assert completed is True
        assert remaining == []

    def test_wait_for_running_jobs_reports_timeout(self):
        import cron.scheduler as sched

        sched._running_job_ids.clear()
        with sched._running_condition:
            sched._running_job_ids.add("stuck-job")
        try:
            completed, remaining = sched.wait_for_running_jobs(timeout=0.01)
            assert completed is False
            assert remaining == ["stuck-job"]
        finally:
            with sched._running_condition:
                sched._running_job_ids.clear()
                sched._running_condition.notify_all()
