...
def test_codex_replay_normalizes_overlong_cross_provider_call_ids(monkeypatch):
    from agent.codex_responses_adapter import (
        _chat_messages_to_responses_input,
        _preflight_codex_input_items,
    )

    long_call_id = "call_" + ("mixed-provider-id-" * 5)
    items = _chat_messages_to_responses_input(
        [
            {
                "role": "assistant",
                ...
                "tool_calls": [
                    {
                        "id": long_call_id,
                        "type": "function",
                        "function": {"name": "supervisor_adapter", "arguments": "{}"},
                    }
                ],
            },
    ...
    assert long_call_id not in str(items)
    ...
    assert len(normalized[0]["call_id"]) <= 64
