...
class TestParseVllmTokenBasedOutputCap:
    ...
    def test_vllm_token_based_format(self):
        # ``at least 65537`` is a lower bound derived from the requested cap,
        # not the exact prompt size.  Halve geometrically instead of shaving
        # one token (plus the caller's margin) per retry.
    ...
    def test_vllm_retry_fits_inside_window(self):
        # Even the conservative lower-bound count fits after the reduction.
        available = parse_available_output_tokens_from_error(self._VLLM_MSG)
        assert available is not None
        assert available + 65537 <= 131072

    def test_vllm_lower_bound_retries_reduce_geometrically(self):
        """Regression: vLLM re-computes ``at least N`` from each cap.

        The old parser returned requested-1 on every round, producing the live
        65536 -> 64511 -> 63486 -> 62461 failure staircase after the caller's
        1024-token reserve.  The parser must make bounded retries converge.
        """
        ctx = 131072
        requested = 65536
        parsed_caps = []
        for _ in range(3):
            lower_bound = ctx - requested + 1
            msg = (
                f"This model's maximum context length is {ctx} tokens. However, "
                f"you requested {requested} output tokens and your prompt "
                f"contains at least {lower_bound} input tokens, for a total of "
                f"at least {ctx + 1} tokens."
            )
            requested = parse_available_output_tokens_from_error(msg)
            assert requested is not None
            parsed_caps.append(requested)

        assert parsed_caps == [32768, 16384, 8192]
