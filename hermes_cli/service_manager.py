...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # on one code path.
            pass
        # Ownership changes may clear setgid bits on both Linux and macOS, so
        # apply the final mode last.  On macOS an unprivileged development run
        # may still have setgid stripped when tmp_path inherited a group the
        # user does not belong to; the remaining 01730 mode is sufficient for
        # the non-s6 test host, while the root-owned s6 container retains 03730.
        path.chmod(mode)
