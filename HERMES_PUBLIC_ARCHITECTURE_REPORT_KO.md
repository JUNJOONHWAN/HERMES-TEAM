...
market, browser-research, operations, report, verification, tool-management,
hermes-repair다. `hermes-repair`는 Hermes 자체의 controller·adapter·shell·router·
supervisor 설정만 수선한다. 일반 제품 코드 작성과 버그 수정은 계속 code 또는
operations에 남으며 저비용 실행기를 사용할 수 있다.
...
| OpenRouter fixed | 선택 | `OPENROUTER_API_KEY` + catalog/tool probe | controller |
| OpenRouter strict free | 선택 | zero-price + text/tool catalog + ordered tool probe | strongest-first controller fallback |
...
OpenRouter 무료 라우터는 무작위 `openrouter/free`를 쓰지 않는다. live catalog에서
`:free`, prompt/completion 0원, text 출력, tools/tool_choice, 최소 context 계약을
모두 통과한 모델만 남긴다. 검증된 강한 순서를 primary로 두고 새 적격 모델은 자동
발견해 후순위에 넣으며, OpenRouter `models` 배열로 rate limit·종료·provider 장애
시 같은 요청 안에서 다음 무료 모델을 순서대로 시도한다.
