...
class TestSchemaBackedHandlers:
    def test_handler_exposes_real_arguments_and_dispatches_them(self):
        from agent.transports.hermes_tools_mcp_server import _make_schema_handler
        ...
        handler = _make_schema_handler(
            "browser_navigate", "Navigate", schema, dispatch
        )

        signature = inspect.signature(handler)
        assert list(signature.parameters) == ["url", "full"]
        assert signature.parameters["url"].default is inspect.Parameter.empty
        assert handler(url="https://example.com", full=True) == "ok"
        assert seen == [
            ("browser_navigate", {"url": "https://example.com", "full": True})
        ]

    def test_built_server_publishes_hermes_schema_without_kwargs(self):
        from agent.transports.hermes_tools_mcp_server import _build_server

        server = _build_server()
        tool = server._tool_manager.get_tool("browser_navigate")
        assert tool is not None
        assert "kwargs" not in (tool.parameters.get("properties") or {})
        assert set(tool.parameters["properties"]) == {"url"}
        assert tool.parameters.get("required") == ["url"]

    def test_built_server_accepts_normal_mcp_kwargs(self, monkeypatch):
        import model_tools
        from agent.transports.hermes_tools_mcp_server import _build_server
