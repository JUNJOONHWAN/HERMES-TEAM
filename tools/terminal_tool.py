...
_CRON_SLEEP_RE = re.compile(
    r"(?:^|[;&|]\s*|\bthen\s+|\bdo\s+)\s*sleep\s+([0-9]+)([smhd]?)\b",
    re.IGNORECASE,
)


def _cron_long_sleep_guard(command: str) -> str | None:
    """Reject long foreground sleeps inside cron-managed agent sessions."""
    if os.getenv("HERMES_CRON_SESSION") != "1":
        return None

    unquoted = _strip_quotes(command)
    for match in _CRON_SLEEP_RE.finditer(unquoted):
        ...
        if seconds > 30:
            return (
                "Cron terminal command uses long sleep. Cron watchdog jobs must "
                "take an action, write status/evidence, and exit; schedule the "
                "next cron tick or run a managed readiness check instead of "
                f"blocking for {seconds}s."
            )
...
def terminal_tool(
    ...
    try:
        ...
        cron_sleep_error = _cron_long_sleep_guard(command)
        if cron_sleep_error:
            return json.dumps({
                "output": "",
                "exit_code": -1,
                "error": cron_sleep_error,
                "status": "error",
            }, ensure_ascii=False)
