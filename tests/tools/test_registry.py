...
class TestBuiltinDiscovery:
    ...
    def test_imports_module_using_tool_registry_alias(self, tmp_path):
        tools_dir = tmp_path / "tools"
        tools_dir.mkdir()
        (tools_dir / "__init__.py").write_text("", encoding="utf-8")
        (tools_dir / "alias_tool.py").write_text(
            "from tools.registry import registry as tool_registry\n"
            "tool_registry.register(name='alias', toolset='x', schema={}, "
        ...
        assert imported == ["tools.alias_tool"]
        mock_import.assert_called_once_with("tools.alias_tool")
