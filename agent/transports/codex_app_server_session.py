...
class _ServerRequestRouting:
    ...
    # Only dispatcher-stamped role-shell capabilities may populate this set.
    # It is deliberately separate from the broad exec/apply-patch bypass: a
    # headless worker can use the MCP servers its immutable role contract
    # granted without silently approving unrelated profile MCPs.
    auto_accept_mcp_elicitation_servers: frozenset[str] = field(
...
class CodexAppServerSession:
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            ...
            capabilities=(
                {"experimentalApi": True} if self._dynamic_tools else None
            ),
        )
    ...
    def _handle_server_request(self, req: dict) -> None:
        ...
        if method == "item/tool/call":
            ...
            # behalf of an MCP server (e.g. tool-call confirmation, OAuth,
            # form data). hermes-tools is our local callback and is always
            # trusted. Bound role workers may additionally accept only the
            # server names stamped into their effective-capability set by the
            # dispatcher. Profile-only or controller MCPs stay fail-closed.
            server_name = str(params.get("serverName") or "").strip()
            trusted_servers = self._routing.auto_accept_mcp_elicitation_servers
            if server_name == "hermes-tools" or server_name in trusted_servers:
    ...
    def _handle_dynamic_tool_call(self, request_id: Any, params: dict) -> None:
        """Execute one app-server dynamic tool through the local supervisor.

        Dynamic tools are an app-server protocol feature, not MCP.  The
        handler itself owns the allowlist; this adapter only validates the
        wire shape and converts the result to DynamicToolCallResponse.
        """
            ...
            self._client.respond(
                request_id,
                {
                    "contentItems": [
                        {
                            "type": "inputText",
                            "text": f"Invalid arguments for {tool_name}: expected object",
                        }
                    ],
                    "success": False,
                },
        ...
        if self._dynamic_tool_handler is None:
            self._client.respond(
                request_id,
                {
                    "contentItems": [
                        {
                            "type": "inputText",
                            "text": f"No Hermes handler for dynamic tool {tool_name}",
                        }
                    ],
                    "success": False,
                },
            ...
            return
        try:
            success, output = self._dynamic_tool_handler(tool_name, arguments)
        ...
        self._client.respond(
            ...
            {
                "contentItems": [
                    {"type": "inputText", "text": str(output)}
                ],
