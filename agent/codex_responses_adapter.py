...
def _normalize_responses_call_id(raw_call_id: Any) -> str:
    """Return a stable Responses-compatible call id.

    Provider switches can replay tool ids minted by another backend.  Some of
    those ids exceed the Codex Responses API's 64-character limit.  Hash only
    the overlong API copy so stored history keeps its original audit id while
    matching function-call and function-output items receive the same safe id.
    """
    value = str(raw_call_id or "").strip()
    if len(value) <= _MAX_RESPONSES_CALL_ID_LENGTH:
        return value
    digest = hashlib.sha256(
        value.encode("utf-8", errors="replace")
    ).hexdigest()
    return f"call_{digest[:59]}"
