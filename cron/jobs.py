...
def _normalize_max_tokens(value: Any) -> Optional[int]:
    """Normalize an optional per-job output-token cap.

    ``None`` or an empty string clears the override. Booleans are rejected
    explicitly because ``bool`` is an ``int`` subclass in Python and silently
    accepting ``True`` as one output token would create a broken job.
    """
    if value is None or value == "":
        return None
    if isinstance(value, bool):
        raise ValueError("Cron job max_tokens must be a positive integer")
    try:
        parsed = int(value)
    except (TypeError, ValueError) as exc:
        raise ValueError("Cron job max_tokens must be a positive integer") from exc
    if parsed <= 0:
        raise ValueError("Cron job max_tokens must be a positive integer")
    return parsed
...
def _normalize_job_record(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Legacy or hand-edited invalid values must not crash list/get/run.
        # Treat them as no override; create/update paths remain strict.
...
def _default_watchdog_contract(
    job_name: Optional[str],
    script: Optional[str],
    no_agent: bool,
) -> Dict[str, Any]:
    """Default contract that makes new cron jobs active watchdogs.

    Domain-specific jobs can strengthen this through their own prompt/script,
    but every job gets the same baseline: evidence, explicit final gate or next
    action, no false completion, and weak-response failure.
    """
    mode = "script_only" if no_agent else (
        "agent_with_script_evidence" if script else "agent_watchdog"
    )
    return {
        "version": 1,
        "mode": mode,
        "status_contract": "report_final_gate_or_next_action",
        "final_gate_required": True,
        "evidence_required": True,
        "empty_response_is_failure": True,
        "one_character_response_is_failure": True,
        "no_false_ok": True,
        "default_next_action": "inspect_evidence_then_continue_or_escalate",
        "created_for": str(job_name or "cron job")[:120],
    }


def _ensure_watchdog_contract(job: Dict[str, Any]) -> Dict[str, Any]:
    normalized = dict(job)
    existing = normalized.get("watchdog_contract")
    base = _default_watchdog_contract(
        normalized.get("name") or normalized.get("prompt"),
        normalized.get("script"),
        bool(normalized.get("no_agent")),
    )
    if isinstance(existing, dict):
        base.update(existing)
    normalized["watchdog_contract"] = base
    return normalized
...
def create_job(
    ...
    Args:
        ...
        max_tokens: Optional positive output-token cap for this job's agent
    ...
    if repeat is not None:
        try:
            ...
            repeat = None  # non-integer, treat as "forever"
    ...
    job = {
        ...
        "watchdog_contract": _default_watchdog_contract(
            name or "cron job",
            normalized_script,
            normalized_no_agent,
        ),
    ...
    with _jobs_lock():
        ...
        for i, job in enumerate(jobs):
            ...
            if "max_tokens" in updates:
                updates["max_tokens"] = _normalize_max_tokens(updates["max_tokens"])
            ...
            jobs[i] = _ensure_watchdog_contract(updated)
            save_jobs(jobs)
