...
_running_condition = threading.Condition(_running_lock)
...
def get_running_job_ids() -> list[str]:
    """Return a stable snapshot of cron jobs currently executing."""
    with _running_condition:
        return sorted(str(job_id) for job_id in _running_job_ids)


def wait_for_running_jobs(timeout: Optional[float] = None) -> tuple[bool, list[str]]:
    """Wait for every dispatched cron job to finish execution and delivery.

    Gateway shutdown uses this before tearing down adapters, MCP servers, or
    the systemd cgroup. Without the barrier, ``KillMode=mixed`` can SIGKILL a
    long-running cron child after ``TimeoutStopSec`` and leave ``jobs.json``
    claiming that a run never happened even though its next run was advanced.

    Returns ``(completed, remaining_job_ids)``. ``timeout=None`` waits until
    the set is empty; systemd provides the outer emergency bound.
    """
    deadline = None if timeout is None else time.monotonic() + max(0.0, timeout)
    with _running_condition:
        while _running_job_ids:
            if deadline is None:
                _running_condition.wait()
                continue
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                break
            _running_condition.wait(remaining)
        remaining_ids = sorted(str(job_id) for job_id in _running_job_ids)
    return not remaining_ids, remaining_ids
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # is a cron delivery. Chat notifications are clean by default; job metadata
    # remains in the cron ledger/web UI. A job-level boolean wins
    # over cron.wrap_response so terse operational signals can remain one line
    # without changing every other cron notification.
    wrap_response = False
    user_cfg = None
    try:
        ...
        pass
    if isinstance(job.get("wrap_response"), bool):
...
def _watchdog_contract_prompt(job: dict) -> str:
    contract = job.get("watchdog_contract")
    # Preserve legacy cron semantics unless a persisted job explicitly opted
    # into the watchdog contract. Newly-created jobs receive a contract from
    # cron.jobs.create_job(); hand-built/legacy records do not.
    if not isinstance(contract, dict) or contract.get("disabled") is True:
        return ""
    job_id = str(job.get("id") or "")
    job_name = str(job.get("name") or job_id or "cron job")
    return (
        "[HERMES CRON WATCHDOG CONTRACT]\n"
        f"Job: {job_name} ({job_id}).\n"
        "You are not a simple scheduled text generator. You are the active "
        "watchdog/manager for this scheduled job.\n"
        "If script output or prior evidence shows pending/running/partial/error, "
        "inspect the evidence, decide whether it is healthy or stale, and take "
        "the safest available recovery action using allowed tools.\n"
        "Do not claim completion from a weak intermediate marker. Completion "
        "requires an explicit final gate, terminal evidence, or a concrete true "
        "blocker with exact reason.\n"
        "Your final response must contain: watchdog_state, final_gate, evidence, "
        "action_taken, next_action, blocker.\n"
        "If there is genuinely nothing to report, use [SILENT]. A blank, "
        "one-character, or content-free response is invalid.\n"
        f"Contract JSON: {json.dumps(contract, ensure_ascii=False, sort_keys=True)}\n"
        "[/HERMES CRON WATCHDOG CONTRACT]\n\n"
    )


def _cron_response_too_weak(text: str) -> bool:
    stripped = (text or "").strip()
    if not stripped:
        return True
    if _is_cron_silence_response(stripped):
        return False
    if len(stripped) <= 1:
        return True
    if stripped.lower() in {"ok", "done", "완료"}:
        return True
    return False
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    watchdog_prompt = _watchdog_contract_prompt(job)
    prompt = cron_hint + prompt
    if watchdog_prompt:
        # Keep runtime evidence before both the user's instruction and the
        # watchdog metadata.  The watchdog contract may repeat a friendly job
        # name derived from the instruction; appending it avoids making that
        # metadata look like the first occurrence of the actual task prompt.
        prompt = prompt + "\n\n" + watchdog_prompt
    if skills is None:
    ...
    try:
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            if _explainer_text and final_response.strip() == _explainer_text.strip():
                ...
                final_response = ""
        _watchdog_contract = job.get("watchdog_contract")
        _watchdog_validation_enabled = (
            isinstance(_watchdog_contract, dict)
            and _watchdog_contract.get("disabled") is not True
        )
        if _watchdog_validation_enabled and _cron_response_too_weak(final_response):
            raise RuntimeError(
                "cron watchdog response too weak: final response did not "
                "contain an auditable status/action/final gate"
            )
