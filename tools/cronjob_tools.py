...
def _format_job(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if job.get("max_tokens") is not None:
        result["max_tokens"] = job["max_tokens"]
    if job.get("no_agent"):
    ...
    try:
        ...
        if normalized == "update":
            ...
            if repeat is not None:
                ...
                try:
                    normalized_repeat = None if int(repeat) <= 0 else int(repeat)
                except (TypeError, ValueError):
                    normalized_repeat = None
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "max_tokens": {
                "type": "integer",
                "minimum": 1,
                "description": "Optional per-job output-token cap. Use a small value such as 1024 for concise alerts and watchdog feedback. Omit to inherit model.max_tokens."
            },
            "script": {
                "type": "string",
