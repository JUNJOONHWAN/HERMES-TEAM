...
class TestAgentConfigSignature:
    ...
    def test_temperature_change_busts_cache(self):
        """Editing model.temperature in config must produce a new signature."""
        from gateway.run import GatewayRunner

        runtime = {"api_key": "k", "base_url": "u", "provider": "p"}
        sig1 = GatewayRunner._agent_config_signature(
            "m", runtime, [], "",
            cache_keys={"model.temperature": 0.1},
        )
        sig2 = GatewayRunner._agent_config_signature(
            "m", runtime, [], "",
            cache_keys={"model.temperature": 0.7},
        )
        assert sig1 != sig2
...
class TestExtractCacheBustingConfig:
    ...
    def test_reads_model_context_length(self):
        ...
        assert out["model.temperature"] is None
    ...
    def test_reads_model_temperature(self):
        ...
        assert out["model.temperature"] == 0.1

    def test_reads_agent_response_language(self):
        from gateway.run import GatewayRunner

        out = GatewayRunner._extract_cache_busting_config(
            {"agent": {"response_language": "ko"}}
        )

        assert out["agent.response_language"] == "ko"
