...
def _strip_anonymous_opencode_zen_authorization(request: Any) -> None:
    """Remove the SDK bearer header from OpenCode Zen's anonymous free API."""
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    anonymous_opencode_zen = bool(
        agent.provider == "opencode-zen"
        and client_kwargs.get("api_key") == "no-key-required"
        and base_url_host_matches(
            str(client_kwargs.get("base_url", "") or ""), "opencode.ai"
        )
    )
    if anonymous_opencode_zen:
        default_headers = dict(client_kwargs.get("default_headers") or {})
        default_headers.setdefault("User-Agent", "HermesAgent/opencode-zen-free")
    ...
    if anonymous_opencode_zen:
        ...
        if not isinstance(event_hooks, dict):
            raise RuntimeError(
                "anonymous OpenCode Zen requires an httpx request hook"
            )
        event_hooks.setdefault("request", []).append(
            _strip_anonymous_opencode_zen_authorization
        )
