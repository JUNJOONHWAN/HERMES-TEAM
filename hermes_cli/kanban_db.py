...
class Task:
    ...
    # Immutable role-shell contract used for supervisor-routed work.  The
    # selected executor/binding live on each task_runs attempt, not here.
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        ...
        return cls(
            ...
            role_shell_id=(
                row["role_shell_id"] if "role_shell_id" in keys else None
            ),
        )
...
class Run:
    ...
    def from_row(cls, row: sqlite3.Row) -> "Run":
        ...
        return cls(
            ...
            metadata=meta,
            error=row["error"],
            role_shell_id=(row["role_shell_id"] if "role_shell_id" in row.keys() else None),
            executor_id=(row["executor_id"] if "executor_id" in row.keys() else None),
            binding_id=(row["binding_id"] if "binding_id" in row.keys() else None),
            receipt_id=(
                int(row["receipt_id"])
                if "receipt_id" in row.keys() and row["receipt_id"] is not None
                else None
            ),
            adapter_override_id=(
                row["adapter_override_id"]
                if "adapter_override_id" in row.keys()
                else None
            ),
        )
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    # The lightweight supervisor is an additive extension of the Kanban
    # narrow waist. Install it after legacy table rebuilds so provenance
    # columns cannot be dropped by a task_runs shape migration.
    from hermes_cli.supervisor_registry import ensure_schema as _ensure_supervisor_schema

    _ensure_supervisor_schema(conn)
...
def create_task(
    ...
    *,
    ...
    adapter_created_by: Optional[str] = None,
) -> str:
    ...
    role_shell_id = str(role_shell_id or "").strip() or None
    if role_shell_id and assignee:
        raise ValueError("assignee and role_shell_id are mutually exclusive")
    adapter_executor_id = str(adapter_executor_id or "").strip() or None
    adapter_executor = None
    role_shell = None
    if adapter_executor_id and not role_shell_id:
        raise ValueError("adapter_executor_id requires role_shell_id")
    if role_shell_id:
        from hermes_cli import supervisor_registry as _supervisor_registry

        _supervisor_registry.ensure_schema(conn)
        role_shell = _supervisor_registry.get_shell(conn, shell_id=role_shell_id)
        if role_shell is None:
            raise ValueError(f"unknown role shell: {role_shell_id}")
        active = _supervisor_registry.get_shell(
            conn, shell_key=role_shell.shell_key
        )
        if active is None or active.id != role_shell.id:
            raise ValueError(
                f"role shell {role_shell_id} is not the active {role_shell.shell_key} version"
            )
        if adapter_executor_id:
            adapter_executor = _supervisor_registry.resolve_executor(
                conn, adapter_executor_id
            )
            if adapter_executor is None:
                raise ValueError(f"unknown executor: {adapter_executor_id}")
            _supervisor_registry._validate_override_executor(
                conn, executor=adapter_executor, shells=[role_shell]
            )
        ...
        try:
            with write_txn(conn):
                ...
                if adapter_executor is not None:
                    override = _supervisor_registry.create_adapter_override_in_txn(
                        conn,
                        scope_type="task",
                        scope_key=task_id,
                        executor_id=adapter_executor.id,
                        mode=adapter_override_mode,
                        reason=adapter_reason,
                        created_by=adapter_created_by or created_by,
                    )
                ...
                _append_event(
                    ...
                    {
                        "assignee": assignee,
                        "role_shell_id": role_shell_id,
                        "adapter_override_id": adapter_override_id,
                        "adapter_executor_id": (
                            adapter_executor.id if adapter_executor is not None else None
                        ),
                        "adapter_override_mode": (
                            adapter_override_mode if adapter_executor is not None else None
                        ),
                        "status": task_status,
    ...
    *,
    ...
    with write_txn(conn):
        ...
        route_row = conn.execute(
            "SELECT role_shell_id FROM tasks WHERE id=? AND status='ready'",
            (task_id,),
        ).fetchone()
        if route_row and route_row["role_shell_id"]:
            from hermes_cli.supervisor_registry import (
                NoEligibleExecutor,
                select_binding,
            )

            try:
                # Selection and claim share this BEGIN IMMEDIATE transaction.
                # The task_runs row created below is therefore the capacity
                # reservation and the authoritative attempt provenance.
                route_selection = select_binding(
                    conn,
                    route_row["role_shell_id"],
                    reserve=True,
                    require_active=False,
                    task_id=task_id,
                )
            except NoEligibleExecutor:
                return None
        ...
        trow = conn.execute(
            "SELECT assignee, max_runtime_seconds, current_step_key, role_shell_id "
            "FROM tasks WHERE id = ?",
            (task_id,),
        ).fetchone()
        run_cur = conn.execute(
            ...
            INSERT INTO task_runs (
                ...
                started_at, role_shell_id, executor_id, binding_id, adapter_override_id
            ) VALUES (?, ?, ?, 'running', ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                task_id,
                ...
                now,
                trow["role_shell_id"] if trow else None,
                route_selection.executor.id if route_selection else None,
                route_selection.binding.id if route_selection else None,
                route_selection.adapter_override_id if route_selection else None,
            ),
        )
        ...
        _append_event(
            conn, task_id, "claimed",
            {
                ...
                "run_id": run_id,
                "role_shell_id": (
                    route_selection.shell.id if route_selection else None
                ),
                "executor_id": (
                    route_selection.executor.id if route_selection else None
                ),
...
def complete_task(
    ...
    *,
    ...
    ``result`` is the canonical user-facing deliverable. For a bound role run
    that omits it, a validated receipt body is promoted; the short ``summary``
    is the final fallback. When ``summary`` is omitted we fall back to the
    canonical result so single-run callers do not have to pass both.
    ...
    delivery_source = "explicit_result" if result and result.strip() else None
    if prepared_receipt is not None and not delivery_source:
        effective_result = receipt_delivery_body(prepared_receipt)
        if effective_result:
            delivery_source = "receipt_outputs"
        elif summary and summary.strip():
            # Bound work must never look empty to the supervisor merely because
            # a worker supplied only the short handoff. The prompt and schema
            # require the exact final answer in result, while this fallback
            # keeps the state machine recoverable if an executor ignores it.
            effective_result = summary
            delivery_source = "summary_fallback"
    ...
    with write_txn(conn):
        ...
        run_id = _end_run(
            ...
            summary=summary if summary is not None else effective_result,
            metadata=metadata,
        )
        ...
        if run_id is None and (summary or metadata or effective_result):
        ...
        if prepared_receipt is not None:
            if run_id is None:
                raise ReceiptValidationError(
                    "bound role completion did not produce a run id"
                )
            from hermes_cli.supervisor_registry import store_receipt_in_txn

            receipt_id = store_receipt_in_txn(
                conn, run_id=run_id, receipt=prepared_receipt
            )
        # Carry the handoff summary in the event payload so gateway
        # notifiers and dashboard WS consumers can render it without a
        # second SQL round-trip. First line only, 400 char cap — the
        # full summary stays on the run row.
        ev_summary = (summary if summary is not None else effective_result) or ""
        ...
        if prepared_receipt is not None:
            completed_payload["receipt_id"] = receipt_id
        if verified_cards:
        ...
        # A successful result-recovery card supersedes blocked execution
        # attempts without deleting their history.  Keep this in the same
        # transaction as completion so the board cannot expose a done recovery
        # alongside permanently-blocked source cards.
        from hermes_cli.supervisor_registry import (
            terminalize_recovery_sources_in_txn,
        )
    ...
    with write_txn(conn):
        ...
        if kind == "dependency":
            ...
            if prepared_receipt is not None:
                if run_id is None:
                    raise ReceiptValidationError(
                        "bound role block did not produce a run id"
                    )
                receipt_id = store_receipt_in_txn(
                    conn, run_id=run_id, receipt=prepared_receipt
                )
            _append_event(
                conn, task_id, "dependency_wait",
                {
                    "reason": reason,
                    "kind": kind,
                    "receipt_id": receipt_id if prepared_receipt is not None else None,
                }, run_id=run_id,
            )
            routed_to = "todo"
...
def archive_task(
    ...
    *,
    ...
    """Archive a card and cancel any host-local worker that owned its run.

    The database transition happens before process termination.  This closes
    both sides of the dispatcher race: a PID recorded first is captured and
    terminated here, while a process that finishes spawning after this commit
    is rejected by :func:`_set_worker_pid` and terminated by the dispatcher.
    """
    archived_run_id: Optional[int] = None
    ...
    if previous_pid is not None:
        with write_txn(conn):
            if not termination.get("terminated"):
                # Preserve the cleanup handle for the next dispatcher tick.
                # This covers both a stubborn local process and an archive
                # request issued on a host that does not own the worker.
                conn.execute(
                    "UPDATE tasks SET worker_pid=?, claim_lock=? "
                    "WHERE id=? AND status='archived' AND worker_pid IS NULL",
                    (previous_pid, previous_claim_lock, task_id),
                )
            _append_event(
                conn,
                task_id,
...
class DispatchResult:
    ...
    skipped_no_binding: list[tuple[str, str, str]] = field(default_factory=list)
    """Role-routed task ids deferred because no executor binding was eligible.
    Each entry is ``(task_id, role_shell_id, reason)``. The task stays ready;
    a later executor heartbeat, capacity release, or binding change retries it."""
...
def _terminate_reclaimed_worker(
    ...
    *,
    ...
    process_group_mode = bool(
        signal_fn is None and os.name != "nt" and hasattr(os, "killpg")
    )

    def _signal(sig: int) -> None:
        if signal_fn is not None:
            signal_fn(int(pid), sig)
        elif process_group_mode:
            # Dispatcher workers are process-group leaders because every
...
def reap_archived_workers(
    conn: sqlite3.Connection,
    *,
    signal_fn=None,
) -> list[str]:
    """Retry cleanup for archived cards whose worker survived cancellation.

    ``archive_task`` keeps the PID + original claim owner only when immediate
    termination cannot be proved.  The dispatch owner retries those handles on
    every tick; a foreign-host handle remains untouched until that host runs.
    """
    reaped: list[str] = []
    rows = conn.execute(
    ...
    for row in rows:
        ...
        with write_txn(conn):
            cur = conn.execute(
                ...
                "WHERE id=? AND status='archived' AND worker_pid=? "
                "AND claim_lock IS ?",
...
def _dispatch_once_locked(
    ...
    *,
    ...
    ready_rows = conn.execute(
        "SELECT id, assignee, role_shell_id FROM tasks "
    ...
    for row in ready_rows:
        ...
        is_role_task = bool(role_shell_id)
        ...
        if is_role_task:
            ...
            try:
                # Preview only. claim_task repeats selection while holding the
                # claim transaction so capacity/provenance remain atomic.
                select_binding(conn, role_shell_id, reserve=False)
            except NoEligibleExecutor as exc:
                result.skipped_no_binding.append(
                    (row["id"], role_shell_id, str(exc))
                )
                continue
            row_assignee = role_shell_id
        if not row_assignee:
        ...
        if _per_profile_cap is not None and not is_role_task:
        ...
        if dry_run:
            ...
            if _per_profile_cap is not None and row_assignee and not is_role_task:
        ...
        if spawn_fn is not None:
            _spawn = spawn_fn
        elif claimed.role_shell_id:
            from hermes_cli.executor_adapter import spawn_bound_task

            def _spawn(task, workspace, *, board=None):
                return spawn_bound_task(conn, task, workspace, board=board)
        else:
        ...
        try:
                    ...
                    with write_txn(conn):
                        if not termination.get("terminated"):
                            conn.execute(
                                ...
                                (int(pid), claimed.claim_lock, claimed.id),
                            )
                        _append_event(
                            conn,
                            claimed.id,
                            "spawn_cancelled_terminal_race",
                            {"pid": int(pid), **termination},
            ...
            result.spawned.append(
                (
                    claimed.id,
                    claimed.role_shell_id or claimed.assignee or "",
                    str(workspace),
                )
            )
            spawned += 1
...
def _default_spawn(
    ...
    *,
    ...
    prompt = getattr(task, "worker_prompt", None) or f"work kanban task {task.id}"
    env = dict(os.environ)
    ...
    # A supervisor-routed task carries immutable role provenance into the
    # child. These values are stamped by the dispatcher, not accepted from
    # task prose, and are used by the completion receipt gate.
    for attr, env_name in (
        ("role_shell_id", "HERMES_ROLE_SHELL_ID"),
        ("executor_id", "HERMES_EXECUTOR_ID"),
        ("binding_id", "HERMES_BINDING_ID"),
        ("timeline_goal_id", "HERMES_TIMELINE_GOAL_ID"),
    ):
        value = getattr(task, attr, None)
        if value:
            env[env_name] = str(value)
    effective_capabilities = getattr(task, "effective_capabilities", None)
    if effective_capabilities is not None:
        env["HERMES_EFFECTIVE_CAPABILITIES"] = ",".join(
            str(item) for item in effective_capabilities if str(item)
        )
    ...
    # For bound role work, effective capabilities are canonical Hermes
    # toolset names and are an upper bound. Never widen back to the executor
    # profile's normal tool surface. Legacy profile tasks keep their existing
    # profile-derived resolution.
    worker_toolsets = (
        sorted(str(item) for item in effective_capabilities if str(item))
        if effective_capabilities is not None
...
def latest_summary(conn: sqlite3.Connection, task_id: str) -> Optional[str]:
    ...
    The worker writes its short handoff to ``task_runs.summary`` via
    ``complete_task(summary=...)``. New supervisor-bound runs also persist a
    canonical ``tasks.result`` from the explicit result or validated receipt,
    while legacy/unbound cards may still have no result. Dashboards and CLI
    "show" views retain this accessor for run history and compatibility.
