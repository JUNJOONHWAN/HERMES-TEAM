...
def _configured_codex_sandbox_mode(
    *,
    codex_home: Optional[str],
    spawn_env: dict[str, str],
) -> Optional[str]:
    """Return an explicitly configured top-level Codex sandbox mode.

    Kanban workers normally receive a narrow ``workspace-write`` override so
    they can update the board without inheriting a permissive operator setup.
    That must not silently discard an operator's explicit
    ``danger-full-access`` choice, though: on hosts where unprivileged user
    namespaces are disabled by AppArmor, forcing the bwrap-backed profile
    makes every command fail before it starts.  Read only the scalar mode from
    the active Codex config and fail closed to the narrow default on any parse
    or filesystem error.
    """
    root = codex_home or spawn_env.get("CODEX_HOME")
    if root:
        config_path = Path(root).expanduser() / "config.toml"
    else:
        home = spawn_env.get("HOME")
        if not home:
            return None
    ...
    try:
        ...
        return None
    mode = payload.get("sandbox_mode") if isinstance(payload, dict) else None
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # Codex sandbox on by default, but preserve an operator's explicit
        # danger-full-access choice. Some hardened Linux hosts deny the user
        # namespaces required by Codex's bwrap sandbox; replacing an explicit
        # full-access config with workspace-write there makes even read-only
        # commands fail before execution. Without an explicit opt-in, add only
        # the Kanban root as an extra writable root as before.
        if spawn_env.get("HERMES_KANBAN_TASK"):
            ...
            configured_sandbox = _configured_codex_sandbox_mode(
