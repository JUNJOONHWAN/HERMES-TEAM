...
def _kst_timestamp(value: Optional[int]) -> Optional[str]:
    if value is None:
        return None
    try:
        return datetime.fromtimestamp(int(value), tz=_KST).strftime(
            "%Y-%m-%d %H:%M:%S KST"
        )
...
def _completion_provenance(conn: sqlite3.Connection, task: Any) -> str:
    ...
    Completion notifications are the operator's decision surface for reruns.
    Keep this lookup best-effort so a legacy/unbound card can still notify, but
    never guess a worker from the role's current primary: only a persisted run
    is accepted as executor provenance.
    """
    if task is None or not getattr(task, "role_shell_id", None):
        return ""
    try:
        ...
        row = conn.execute(
            "SELECT id,role_shell_id,executor_id,binding_id,adapter_override_id "
            "FROM task_runs WHERE task_id=? ORDER BY id DESC LIMIT 1",
            (str(task.id),),
        ).fetchone()
        if row is None or not row["executor_id"]:
            return ""
        shell = registry.get_shell(conn, shell_id=row["role_shell_id"])
        executor = registry.get_executor(conn, str(row["executor_id"]))
        if shell is None or executor is None:
            return ""

        provenance_row = conn.execute(
            "SELECT payload FROM task_events WHERE task_id=? "
            "AND kind='supervisor_provenance' ORDER BY id DESC LIMIT 1",
            (str(task.id),),
        ).fetchone()
        provenance_payload: dict[str, Any] = {}
        if provenance_row is not None and provenance_row["payload"]:
            try:
                parsed = json.loads(provenance_row["payload"])
                if isinstance(parsed, dict):
                    provenance_payload = parsed
            except (TypeError, ValueError, json.JSONDecodeError):
                provenance_payload = {}
        controller_runtime = provenance_payload.get("runtime")
        controller_override = provenance_payload.get("override")
        if not isinstance(controller_runtime, dict):
            controller_selection = registry.resolve_controller_selection(
                conn,
                session_id=(str(task.session_id) if task.session_id else None),
            )
            if controller_selection is None:
                controller_runtime = registry.controller_runtime_descriptor()
                controller_override = None
            else:
                current_override, controller_adapter = controller_selection
                controller_runtime = registry.controller_adapter_runtime_descriptor(
                    controller_adapter
                )
                controller_override = registry._controller_override_dict(
                    current_override
                )
        executor_runtime = registry.executor_runtime_descriptor(executor)
        ...
        role_label = {
            ...
            "report": "리포트",
            "verification": "검증",
        }.get(shell.shell_key, shell.shell_key)
        lines = [
            f"담당: Hermes {controller_runtime['display_label']}",
            f"실행: {role_label} → {executor.name} {executor_runtime['display_label']}",
            f"기록: run {int(row['id'])}",
        ]

        route_parts: list[str] = []
        run_override_id = row["adapter_override_id"]
        if run_override_id:
            run_override = registry.get_adapter_override(conn, str(run_override_id))
            if run_override is not None:
                state = "active" if run_override.active() else "used/expired"
                detail = f"task {run_override.mode} {state}"
                expiry = _kst_timestamp(run_override.expires_at)
                if expiry:
                    detail += f" until {expiry}"
                route_parts.append(detail)
        shell_overrides = registry.list_adapter_overrides(
            conn,
            scope_type="shell",
            scope_key=shell.shell_key,
        )
        if shell_overrides:
            shell_override = shell_overrides[0]
            detail = f"role {shell_override.mode} active"
            expiry = _kst_timestamp(shell_override.expires_at)
            if expiry:
                detail += f" until {expiry}"
            route_parts.append(detail)
        if isinstance(controller_override, dict):
            detail = f"supervisor {controller_override.get('mode') or 'override'} selected"
            expiry = _kst_timestamp(controller_override.get("expires_at"))
            if expiry:
                detail += f" until {expiry}"
            route_parts.append(detail)
        if route_parts:
            lines.append("전환: " + "; ".join(route_parts))
        return "\n".join(lines)
    except Exception:
        logger.warning(
            "kanban notifier: failed to render responsibility chain for %s",
        ...
        while self._running:
            try:
                    ...
                    for board_meta in boards:
                        ...
                        try:
                            ...
                            for sub in subs:
                                ...
                                provenance = _completion_provenance(conn, task)
                                logger.debug(
                ...
                for d in deliveries:
                    ...
                    # last delivered event, not to the beginning (which would
                    # duplicate already-delivered Telegram messages).
                    ...
                    for ev in d["events"]:
                        ...
                        if kind == "completed":
                            ...
                            # for legacy/unbound rows without a stored result.
                            handoff = ""
                            if task and task.result and task.result.strip():
                                handoff = f"\n\n{task.result.strip()}"
                            elif ev.payload and ev.payload.get("summary"):
                                handoff = f"\n\n{str(ev.payload['summary']).strip()}"
                            provenance_block = (
                                f"\n\n{d['provenance']}"
                                if d.get("provenance")
                                else ""
                            ...
                            try:
                                ...
                                if _session_key:
                                    ...
                                    _source = SessionSource(
                                        ...
                                        chat_id=sub["chat_id"],
                                        chat_type=(
                                            str(sub.get("chat_type") or "").strip()
                                            or "group"
                                        ),
    ...
    async def _deliver_kanban_artifacts(
        ...
        *,
        ...
        Sources scanned, in priority order:
          ...
          3. ``task.result`` (canonical deliverable and path fallback)
        ...
        # 3. Paths embedded in the canonical task.result deliverable.
