...
def test_adapter_api_lists_switches_and_exposes_task_provenance(client):
    conn = kb.connect()
    try:
        ...
        task_id = kb.create_task(conn, title="adapter task", role_shell_id=shell.id)
    finally:
        conn.close()

    listed = client.get("/api/plugins/kanban/adapters")
    assert listed.status_code == 200, listed.text
    assert listed.json()["shells"][0]["primary"][0]["executor_id"] == executor.id
    assert listed.json()["control_slot_count"] == 2
    assert listed.json()["control_slots"][0]["slot_key"] == "hermes"
    assert "runtime" in listed.json()["control_slots"][1]
    assert "recent_tasks" in listed.json()

    switched = client.post(
        "/api/plugins/kanban/adapters/switch",
        json={
            "target": task_id,
            "scope_type": "task",
            "executor_id": executor.id,
    ...
    override_id = switched.json()["override_id"]

    detail = client.get(f"/api/plugins/kanban/tasks/{task_id}")
    assert detail.status_code == 200, detail.text
    assert detail.json()["adapter"]["effective_override"]["override_id"] == override_id
    assert detail.json()["events"][-1]["kind"] == "adapter_override_created"

    cleared = client.post(
        "/api/plugins/kanban/adapters/clear",
        json={"override_id": override_id, "reason": "restore"},
    )
    assert cleared.status_code == 200, cleared.text


def test_dashboard_bundle_renders_adapter_control_and_run_provenance():
    repo_root = Path(__file__).resolve().parents[2]
    js = (repo_root / "plugins" / "kanban" / "dashboard" / "dist" / "index.js").read_text()

    assert "function AdapterPanel(props)" in js
    assert "function runtimeLabel(value)" in js
    assert "Hermes Control Tower (hermes)" in js
    assert "control slots" in js
    assert "Recent adapter tasks" in js
    assert "Executor adapters" in js
    assert "reasoning ${runtime.reasoning_effort" in js
    assert "function TaskAdapterSection(props)" in js
    assert "Re-request revision" in js
    assert "adapter=${r.executor_id}" in js
