...
def test_anonymous_opencode_auxiliary_strips_placeholder_authorization():
    from agent.auxiliary_client import _create_openai_client

    fake_http = SimpleNamespace(event_hooks={"request": []})
    fake_client = MagicMock()
    with (
        patch(
            "agent.auxiliary_client._openai_http_client_kwargs",
            return_value={"http_client": fake_http},
        ),
        patch("agent.auxiliary_client.OpenAI", return_value=fake_client) as openai,
    ):
        assert _create_openai_client(
            api_key="no-key-required",
            base_url="https://opencode.ai/zen/v1",
        ) is fake_client

    request = SimpleNamespace(
        headers={"Authorization": "Bearer no-key-required", "Accept": "json"}
    )
    assert len(fake_http.event_hooks["request"]) == 1
    fake_http.event_hooks["request"][0](request)
    assert "Authorization" not in request.headers
    assert request.headers["Accept"] == "json"
    assert openai.call_args.kwargs["default_headers"]["User-Agent"].startswith(
        "HermesAgent/"
    )


def test_auto_auxiliary_keeps_request_scoped_provider_model_pair():
    primary = MagicMock()
    with (
        patch("agent.auxiliary_client._read_main_model", return_value="gpt-5.6-sol"),
        patch(
            "agent.auxiliary_client._resolve_auto",
            return_value=(primary, "hy3-free"),
        ) as resolve_auto,
    ):
        client, model = resolve_provider_client(
            "auto",
    ...
    assert model == "hy3-free"
    resolve_auto.assert_called_once()


def test_locked_compression_fallback_uses_codex_without_generic_discovery():
    primary = MagicMock()
    ...
    def cached(provider, model=None, **_kwargs):
        if provider == "auto":
            return primary, "hy3-free"
        if provider == "openai-codex":
            return fallback, "gpt-5.6-sol"
        raise AssertionError(f"unexpected provider: {provider}")

    with (
        patch(
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("auto", "hy3-free", None, None, None),
        ),
        patch("agent.auxiliary_client._get_cached_client", side_effect=cached),
        patch("agent.auxiliary_client._is_auth_error", return_value=True),
        patch("agent.auxiliary_client._try_payment_fallback") as generic_fallback,
    ):
        result = call_llm(
            task="compression",
            main_runtime={
    ...
    sent = fallback.chat.completions.create.call_args.kwargs
    assert sent.get("max_tokens", sent.get("max_completion_tokens")) == 12_000
