...
# ===========================================================================
# Text cleaning for TTS — strips markdown, emojis, and noisy artifacts
# ===========================================================================
import re

def _clean_for_tts(text: str) -> str:
    """Remove markdown, emojis, brackets, and other TTS-unfriendly chars."""
    # Remove code blocks (``` ... ```)
    ...
    # Remove HTML tags
    text = re.sub(r'<[^>]+>', '', text)
    # Remove URL patterns (http://, https://, www.)
    text = re.sub(r'https?://\S+', '', text)
    text = re.sub(r'www\.\S+', '', text)
    # Remove email patterns
    text = re.sub(r'\S+@\S+\.\S+', '', text)
    # Remove common emoji patterns (broad Unicode emoji ranges)
    ...
    # Remove repeated punctuation (!!!, ???, etc.) → single
    text = re.sub(r'([!?.])\1+', r'\1', text)
    # Remove Korean/English brackets with content: [...], (...), 《...》
    text = re.sub(r'[【\[（(〈<].*?[】\]）)>]', '', text)
...
async def _generate_edge_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    """
    # Clean text before TTS — removes markdown, emojis, noise
