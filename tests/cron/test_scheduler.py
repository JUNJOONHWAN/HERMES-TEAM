...
class TestDeliverResultWrapping:
    ...
    def test_delivery_skips_wrapping_for_one_job_only(self):
        """A job-level false emits raw content even when the global default wraps."""
        from gateway.config import Platform
        ...
        with patch("gateway.config.load_gateway_config", return_value=mock_cfg), \
            ...
            _deliver_result(job, "✅ Hermes heartbeat 정상")

        sent_content = send_mock.call_args.kwargs.get("content") or send_mock.call_args[0][-1]
        assert sent_content == "✅ Hermes heartbeat 정상"
        assert "Cronjob Response" not in sent_content
        assert "To stop or manage" not in sent_content

    def test_delivery_job_wrap_true_overrides_global_false(self):
        """An explicit job-level true preserves the standard wrapper."""
        from gateway.config import Platform
        ...
        with patch("gateway.config.load_gateway_config", return_value=mock_cfg), \
            ...
            _deliver_result(job, "Report output")
...
class TestRunJobWakeGate:
    ...
    def test_job_max_tokens_is_forwarded_to_agent(self):
        """A concise cron cap must override the global model output budget."""
        import cron.scheduler as scheduler
        ...
        agent = MagicMock()
        agent.run_conversation = MagicMock(return_value={
            "final_response": "상태: 확인 필요", "messages": []
        })
        with patch.object(scheduler, "_run_job_script",
                          return_value=(True, '{"wakeAgent": true}')), \
             patch("run_agent.AIAgent", return_value=agent) as agent_cls:
            success, _, _, err = scheduler.run_job(job)

        assert success is True
        assert err is None
        assert agent_cls.call_args.kwargs["max_tokens"] == 1024
