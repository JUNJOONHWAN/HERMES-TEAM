...
class TestJobCRUD:
    ...
    def test_create_persists_optional_max_tokens(self, tmp_cron_dir):
        job = create_job(
            prompt="Concise alert",
            schedule="every 1h",
            max_tokens=1024,
        )

        assert job["max_tokens"] == 1024
        assert get_job(job["id"])["max_tokens"] == 1024

    @pytest.mark.parametrize("value", [0, -1, True, "invalid"])
    def test_create_rejects_invalid_max_tokens(self, tmp_cron_dir, value):
        with pytest.raises(ValueError, match="positive integer"):
            create_job(prompt="Alert", schedule="every 1h", max_tokens=value)
...
class TestUpdateJob:
    ...
    def test_update_sets_and_clears_max_tokens(self, tmp_cron_dir):
        job = create_job(prompt="Alert", schedule="every 1h")

        updated = update_job(job["id"], {"max_tokens": 2048})
        assert updated["max_tokens"] == 2048

        cleared = update_job(job["id"], {"max_tokens": None})
