...
def _merge_model_temperature_request_override(agent, model_config: Dict[str, Any]) -> None:
    """Apply an explicit ``model.temperature`` without clobbering callers.

    Provider defaults remain untouched when the config key is absent.  An
    explicit per-request override wins over the global config value.
    """
    if not isinstance(model_config, dict) or "temperature" not in model_config:
        return
    ...
    configured = model_config.get("temperature")
...
def init_agent(
    ...
    try:
        ...
        _agent_cfg = {}
    # A supervisor root never refreshes or imports MCP tools between turns.
    # Executors run in separate profile/command processes and load their own
    # domain surfaces; the central agent sees only Kanban/control receipts.
    from hermes_cli.supervisor_registry import supervisor_root_enabled

    agent._supervisor_mode = supervisor_root_enabled(_agent_cfg)
    if agent._supervisor_mode:
        agent._skip_mcp_refresh = True
        if "supervisor" in set(enabled_toolsets or []):
            from agent.supervisor_contract import SUPERVISOR_CONTROL_TOOL_NAMES

            _missing_supervisor_tools = sorted(
                SUPERVISOR_CONTROL_TOOL_NAMES - agent.valid_tool_names
            )
            if _missing_supervisor_tools:
                raise RuntimeError(
                    "Hermes supervisor control plane failed closed; missing tools: "
                    + ", ".join(_missing_supervisor_tools)
                )
    try:
        agent._tool_guardrails = ToolCallGuardrailController(
    ...
    # Optional fail-closed guard for model code-switching in user-visible
    # prose.  Keep the normalized value on the cached agent; gateway cache
    # invalidation includes this config key so edits take effect immediately.
    from agent.response_language import normalize_response_language
    _configured_response_language = _agent_section.get("response_language", "")
    agent._response_language = normalize_response_language(
        _configured_response_language
    )
    if _configured_response_language and not agent._response_language:
        _ra().logger.warning(
            "Ignoring unsupported agent.response_language value: %r",
            _configured_response_language,
        )
    ...
    try:
        _merge_model_temperature_request_override(agent, _model_cfg)
    except (TypeError, ValueError):
        _config_temperature = (
            _model_cfg.get("temperature") if isinstance(_model_cfg, dict) else None
        )
        _ra().logger.warning(
        ...
        print(
            ...
            f"  Must be a finite number between 0 and 2.\n"
            f"  Falling back to provider default.\n",
            file=sys.stderr,
        )
    agent._session_init_model_config["temperature"] = agent.request_overrides.get(
        "temperature"
    )
    ...
    if agent._supervisor_mode:
        # Compression is part of the controller safety boundary.  It follows
        # the same audited fallback chain as the turn (normally HY3 -> Codex)
        # and must not wander into auxiliary auto-discovery/OpenRouter.
        agent.context_compressor.locked_fallback_runtimes = list(
            agent._fallback_chain
        )
