...
flowchart TD
    ...
    K --> A6["검증 역할 어댑터"]
    K --> A7["멀티툴 역할 어댑터"]
    K --> A8["Hermes 수선 역할 어댑터"]
    A1 & A2 & A3 & A4 & A5 & A6 & A7 & A8 --> E["실행 에이전트 풀<br/>Codex · OpenCode · Grok · 로컬 모델 등"]
...
| 역할 어댑터 | 업무 종류와 권한이 고정된 논리 슬롯 | 총 8개이며 역할 자체는 모델과 분리된다 |
...
Shell을 사용하고 `gpt-5.6-sol/high` 실행자로 고정한다. 경량 컨트롤러가 직접 파일이나
서비스를 고치는 것은 두 경로 모두 감독 계약 위반이다.
