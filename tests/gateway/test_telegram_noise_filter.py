...
def test_chat_suppresses_successful_fallback_chatter():
    assert _prepare_gateway_status_message(
        Platform.TELEGRAM,
        "warn",
        "🔄 Primary model failed — switching to fallback: codex",
    ) is None


def test_chat_rewrites_raw_compression_abort_without_internal_details():
    raw = (
        "⚠ Compression aborted: Error code: 402; requested 65536 tokens. "
