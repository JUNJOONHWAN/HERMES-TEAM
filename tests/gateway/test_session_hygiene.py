...
async def test_session_hygiene_warns_user_when_compression_aborts(monkeypatch, tmp_path):
    ...
    warning_messages = [s for s in adapter.sent if "대화 정리 실패" in s["content"]]
    assert len(warning_messages) == 1, (
        f"Expected 1 compression-aborted warning, got {len(warning_messages)}: {adapter.sent}"
    )
    warn = warning_messages[0]
    # Raw provider/model details stay in logs; chat only states the operator
    # invariant that the original content was preserved.
    assert "404" not in warn["content"]
    assert "내용 보존됨" in warn["content"]
...
async def test_session_hygiene_informs_user_when_aux_model_fails_but_recovers(monkeypatch, tmp_path):
    ...
    # Successful automatic recovery is silent. Provider/model/error details
    # stay in logs instead of producing another operator bubble.
    assert adapter.sent == []

    FakeCompressAgentWithAuxRecovery.last_instance.close.assert_called_once()


def test_hygiene_constructor_runtime_strips_controller_metadata():
    from gateway.run import _hygiene_agent_constructor_runtime

    runtime = {
        "provider": "opencode-zen",
        "api_key": "no-key-required",
        "base_url": "https://opencode.ai/zen/v1",
        "_controller_override": {"override_id": "x"},
        "_controller_fallback_chain": [
            {"provider": "openai-codex", "model": "gpt-5.6-sol"}
        ],
    }
    assert _hygiene_agent_constructor_runtime(runtime) == {
        "provider": "opencode-zen",
        "api_key": "no-key-required",
        "base_url": "https://opencode.ai/zen/v1",
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "  supervisor_hygiene_hard_message_limit: 10\n"
        ),
    ],
)
async def test_session_hygiene_honors_configurable_hard_message_limits(
    monkeypatch, tmp_path, config_text
):
    """General and supervisor-specific hard limits override the default.
