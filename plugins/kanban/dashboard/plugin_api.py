...
def _run_dict(r: kanban_db.Run) -> dict[str, Any]:
    ...
    return {
        ...
        "metadata": r.metadata,
        "error": r.error,
        "role_shell_id": r.role_shell_id,
        "executor_id": r.executor_id,
        "binding_id": r.binding_id,
        "adapter_override_id": r.adapter_override_id,
...
def get_task(
    ...
    try:
        ...
        return {
            ...
            "adapter": supervisor_registry.inspect_task_adapter(conn, task_id),
        }
    finally:
        conn.close()
...
# ---------------------------------------------------------------------------
# Supervisor adapter control plane — same SQLite ledger as CLI + Telegram
# ---------------------------------------------------------------------------

class AdapterAssignBody(BaseModel):
    shell: str
    executor_id: str
...
class AdapterSwitchBody(BaseModel):
    target: str
    executor_id: str
    mode: str
...
class AdapterClearBody(BaseModel):
    override_id: str
    reason: Optional[str] = None


class AdapterRerunBody(BaseModel):
    executor_id: str
    reason: Optional[str] = None
...
def get_adapters(
    ...
    conn = _conn(board=board)
    try:
        return supervisor_registry.adapter_registry_view(
            conn, history_limit=history_limit
        )
    finally:
        conn.close()


@router.post("/adapters/assign")
def assign_adapter(payload: AdapterAssignBody, board: Optional[str] = Query(None)):
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
        binding = supervisor_registry.assign_adapter(
            conn,
            shell_value=payload.shell,
            executor_value=payload.executor_id,
    ...
    except supervisor_registry.SupervisorRegistryError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    finally:
        conn.close()


@router.post("/adapters/switch")
def switch_adapter(payload: AdapterSwitchBody, board: Optional[str] = Query(None)):
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
        override = supervisor_registry.create_adapter_override(
            conn,
        ...
        return {"switched": True, **supervisor_registry._override_dict(override)}
    except supervisor_registry.SupervisorRegistryError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    finally:
        conn.close()
...
@router.post("/adapters/clear")
def clear_adapter(payload: AdapterClearBody, board: Optional[str] = Query(None)):
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
        cleared = supervisor_registry.clear_adapter_override(
            conn,
            payload.override_id,
        ...
        return {"cleared": True, "override_id": payload.override_id}
    finally:
        conn.close()


@router.get("/tasks/{task_id}/adapter")
def inspect_task_adapter(task_id: str, board: Optional[str] = Query(None)):
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
        return supervisor_registry.inspect_task_adapter(conn, task_id)
    except supervisor_registry.SupervisorRegistryError as exc:
...
@router.post("/tasks/{task_id}/adapter-rerun")
def rerun_task_with_adapter(
    task_id: str,
    payload: AdapterRerunBody,
    board: Optional[str] = Query(None),
):
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
        return supervisor_registry.reissue_task_with_adapter(
            conn,
            task_id=task_id,
            executor_value=payload.executor_id,
            reason=payload.reason,
