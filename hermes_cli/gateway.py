...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    # systemd's TimeoutStopSec must cover both the interactive-agent drain and
    # the longest built-in cron-script timeout. Cron execution is intentionally
    # drained before adapters/MCP processes are torn down; a 90s service bound
    # used to SIGKILL hour-long market jobs and lose their completion record.
    # The scheduler's default script timeout is 3600s. Five minutes of
    # headroom covers delivery, adapter teardown, and state flushing.
    _drain_timeout = int(_get_restart_drain_timeout() or 0)
    restart_timeout = max(max(60, _drain_timeout) + 30, 3600 + 300)
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None:
        scope_label = _service_scope_label(system).capitalize()
        svc = get_service_name()
        # A no-agent cron script may run for the scheduler's full one-hour
        # timeout.  Keep the CLI-side SIGUSR1 wait aligned with the generated
        # systemd TimeoutStopSec so an operator restart cannot fall through to
        # a hard systemctl restart while cron is still draining.
        drain_timeout = max(_get_restart_drain_timeout(), 3600 + 300)

        print(f"⏳ {scope_label} service restarting gracefully (PID {pid})...")
        if _graceful_restart_via_sigusr1(pid, drain_timeout + 5):
            # Restart=always/RestartForceExitStatus=75 owns the relaunch.
            # Issuing systemctl restart here races the automatically spawned
            # replacement and can stop it a second time.
            if _wait_for_systemd_service_restart(system=system, previous_pid=pid):
                return
            if _systemd_service_is_start_limited(system=system):
                return
        elif _wait_for_systemd_service_restart(
            system=system,
            previous_pid=pid,
            timeout=10.0,
        ):
            # Some supervisors keep the old PID observable briefly even
            # after committing the replacement MainPID.  The replacement
            # runtime is authoritative; do not force a second restart.
            return
