...
async def test_notifier_unsubs_after_abnormal_events(kind, kanban_home):
    """
    ``gave_up`` is user-actionable and sends one concise notification.
    ``crashed`` / ``timed_out`` are internal retry noise and remain silent.
    All three advance the cursor without deleting the subscription so a later
    real completion or terminal failure still reaches the user.
    """
    import hermes_cli.kanban_db as kb
    from gateway.run import GatewayRunner
    from gateway.config import Platform
    ...
    if kind == "gave_up":
        fake_adapter.send.assert_called_once()
        assert "실행 실패" in fake_adapter.send.call_args[0][1]
    else:
...
async def test_notifier_second_blocked_delivers(kanban_home):
    ...
    blocked_deliveries = [m for m in delivered_msgs if "중단" in m]
...
@pytest.mark.asyncio
async def test_notifier_partial_batch_failure_does_not_duplicate_prior_send(
    kanban_home,
):
    """Retry only the failed suffix when a claimed event batch partly sends."""
    from gateway.config import Platform
    from gateway.run import GatewayRunner

    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="two terminal updates", assignee="worker1")
        kb.add_notify_sub(conn, task_id=tid, platform="telegram", chat_id="chat1")
    ...
    assert sum("first" in message for message in attempts) == 1
    assert sum("second" in message for message in attempts) == 2
    assert sum("first" in message for message in successes) == 1
    assert sum("second" in message for message in successes) == 1
...
async def test_notifier_skips_subscription_owned_by_other_profile(kanban_home):
    ...
    try:
        ...
        initial_cursor = int(kb.list_notify_subs(conn, tid)[0]["last_event_id"])
    ...
    try:
        ...
        conn.close()
    assert len(subs) == 1
    assert int(subs[0]["last_event_id"]) == initial_cursor, (
