...
def _assert_event_dir_mode(path) -> None:
    """Assert the s6 event mode, allowing Darwin to strip setgid in /tmp.

    macOS creates pytest temp directories with a system group that an
    unprivileged developer cannot re-apply as setgid.  The launchd runtime
    never uses this s6 helper; Linux's live-container test remains the strict
    production proof for 03730.
    """
    import stat
    import sys
    ...
    assert actual in expected, (
        f"event/ mode = {oct(path.stat().st_mode)}, want one of "
        f"{', '.join(oct(mode) for mode in sorted(expected))}"
    )
