...
(function () {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "hermes-kanban flex flex-col gap-4" },
        ...
        h(AdapterPanel, { boardSlug: board, onRefresh: loadBoard }),
        h(AttentionStrip, {
          boardData,
  ...
  // Shared role-shell adapter ledger. Telegram, CLI, dispatcher, and this
  // panel all read/write the same supervisor tables in kanban.db.
  function runtimeLabel(value) {
    const runtime = (value && value.runtime) || value || {};
    if (runtime.display_label) return runtime.display_label;
    return [runtime.backend_label || runtime.backend, runtime.model,
      runtime.reasoning_effort].filter(Boolean).join(" · ") || "runtime unavailable";
  }

  function AdapterPanel(props) {
    const [expanded, setExpanded] = useState(false);
    ...
    const load = useCallback(function () {
      return SDK.fetchJSON(withBoard(`${API}/adapters?history_limit=100`, props.boardSlug))
        .then(function (value) { setData(value); setMsg(null); })
        .catch(function (err) {
          setMsg({ ok: false, text: "Adapter load failed: " + parseApiErrorMessage(err) });
        });
    }, [props.boardSlug]);
    ...
    const mutate = function (path, body, successText) {
      setBusy(true); setMsg(null);
      return SDK.fetchJSON(withBoard(`${API}${path}`, props.boardSlug), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      }).then(function () {
        ...
        setMsg({ ok: false, text: parseApiErrorMessage(err) });
      }).finally(function () { setBusy(false); });
    };

    const pick = function (key, value) {
      setSelected(function (prev) { return Object.assign({}, prev, { [key]: value }); });
    };
    const executors = (data && data.executors) || [];
    const executorOptions = executors.map(function (e) {
      return h(SelectOption, { key: e.executor_id, value: e.executor_id },
        `${e.display_label || `${e.name} · ${runtimeLabel(e)}`} · ${e.health_state}${e.enabled ? "" : " · disabled"}`);
    });
    const activeCount = data ? (data.active_overrides || []).length : 0;
    const header = expanded
      ? "▾ Kanban adapters"
      : `▸ Kanban adapters · ${data ? data.control_slot_count : "…"} control slots · ${activeCount} override(s)`;

    if (!expanded) {
      return h("button", {
        type: "button",
        onClick: function () { setExpanded(true); },
        className: "text-left text-xs underline text-muted-foreground hover:text-foreground",
        title: "Inspect and switch role/task executors with an auditable Kanban history",
      }, header);
    }

    const globalOverride = data && data.global_override;
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        h("div", { className: "flex items-center justify-between gap-2" },
          h("button", {
            type: "button", onClick: function () { setExpanded(false); },
            className: "text-sm font-medium underline-offset-2 hover:underline",
          }, header),
          h(Button, { size: "sm", onClick: load, disabled: busy }, "Reload"),
        ),
        h("div", { className: "text-[11px] text-muted-foreground" },
          "Routing order: task override → role override → all override → primary owner → candidates. Forced adapters never silently fall back."),
        msg ? h("div", {
          className: msg.ok ? "hermes-kanban-msg-ok" : "hermes-kanban-msg-err",
        }, msg.text) : null,
        data && data.controller ? h("div", {
          className: "rounded border p-2 grid gap-1 lg:grid-cols-[180px_1fr_280px]",
        },
          h("div", null,
            h("div", { className: "text-xs font-semibold" }, "Hermes Control Tower (hermes)"),
            h("div", { className: "text-[10px] text-muted-foreground" },
              "central controller · delegation only")),
          h("div", { className: "text-xs font-mono" }, runtimeLabel(data.controller)),
          h("div", { className: "text-[10px] text-muted-foreground" },
            `profile: ${(data.controller.runtime || {}).profile || "default"} · controls: ${(data.controller.controls || []).join(", ")}`),
        ) : null,
        data && executors.length ? h("details", { className: "rounded border p-2", open: true },
          h("summary", { className: "text-xs font-semibold cursor-pointer" },
            `Executor adapters (${executors.length})`),
          h("div", { className: "grid gap-2 mt-2 lg:grid-cols-2" },
            executors.map(function (executor) {
              const runtime = executor.runtime || {};
              return h("div", { key: executor.executor_id, className: "rounded border p-2" },
                h("div", { className: "text-xs font-semibold font-mono" }, executor.name || executor.executor_id),
                h("div", { className: "text-[10px] text-muted-foreground" },
                  `${executor.executor_id} · ${executor.adapter_type || "adapter"} · capacity ${executor.capacity}`),
                h("div", { className: executor.health_state === "healthy"
                  ? "text-[10px] text-emerald-600" : "text-[10px] text-amber-600" },
                  `${executor.health_state || "unknown"} · ${executor.enabled ? "enabled" : "disabled"}`),
                h("div", { className: "text-[10px] font-mono mt-1" }, runtimeLabel(executor)),
                h("div", { className: "text-[10px] text-muted-foreground" },
                  `provider ${runtime.provider || "-"} · model ${runtime.model || "-"} · reasoning ${runtime.reasoning_effort || "-"}`),
                h("div", { className: "text-[10px] text-muted-foreground" },
                  `backend ${runtime.backend_label || runtime.backend || "-"} · profile ${runtime.profile || "-"} · endpoint ${runtime.endpoint || "-"}`),
                executor.description ? h("div", { className: "text-[10px] mt-1" }, executor.description) : null,
              );
            }),
          ),
        ) : null,
        data ? h("div", { className: "rounded border p-2 flex flex-wrap items-center gap-2" },
          h("span", { className: "text-xs font-semibold" }, "Emergency ALL"),
          h(Select, Object.assign({
            value: selected.all || "", className: "h-8 min-w-[220px]",
          }, selectChangeHandler(function (v) { pick("all", v); })),
            h(SelectOption, { value: "" }, "Select executor"), executorOptions),
          h(Button, {
            size: "sm", variant: "outline", disabled: busy || !selected.all,
            onClick: function () {
              mutate("/adapters/switch", {
                target: "all", scope_type: "all", executor_id: selected.all,
                mode: "temporary", duration_seconds: 3600,
                reason: "dashboard emergency one-hour switch",
              }, "All roles switched for one hour.");
            },
          }, "All · 1h"),
          h(Button, {
            size: "sm", variant: "outline", disabled: busy || !selected.all,
            onClick: function () {
              if (!window.confirm("Permanently override every role with this executor until cleared?")) return;
              mutate("/adapters/switch", {
                target: "all", scope_type: "all", executor_id: selected.all,
                mode: "permanent", reason: "dashboard emergency permanent switch",
              }, "All roles switched permanently.");
            },
          }, "All · permanent"),
          globalOverride ? h("span", { className: "text-xs text-amber-600" },
            `ACTIVE ${globalOverride.executor_id} · ${globalOverride.mode}`) : null,
          globalOverride ? h(Button, {
            size: "sm", variant: "outline", disabled: busy,
            onClick: function () {
              mutate("/adapters/clear", { override_id: globalOverride.override_id,
                reason: "dashboard restore all defaults" }, "Global override cleared.");
            },
          }, "Restore all") : null,
        ) : null,
        data ? h("div", { className: "flex flex-col gap-2" },
          data.shells.map(function (shell) {
            const primary = (shell.primary || []).map(function (x) { return x.display_label || x.executor_name || x.executor_id; }).join(", ") || "—";
            const candidates = (shell.candidates || []).map(function (x) { return x.display_label || x.executor_name || x.executor_id; }).join(", ") || "—";
            const key = shell.shell_key;
            const active = shell.active_override;
            return h("div", { key: key, className: "rounded border p-2 grid gap-2 lg:grid-cols-[180px_1fr_280px]" },
              h("div", null,
                h("div", { className: "text-xs font-semibold" }, `${shell.name} (${key})`),
                h("div", { className: "text-[10px] font-mono" },
                  `current: ${(shell.effective_executor && shell.effective_executor.display_label) || "unbound"} [${shell.selection_source || "-"}]`),
                h("div", { className: "text-[10px] text-muted-foreground" }, `primary: ${primary}`),
                h("div", { className: "text-[10px] text-muted-foreground" }, `candidates: ${candidates}`),
                active ? h("div", { className: "text-[10px] text-amber-600" },
                  `override: ${active.executor_id} · ${active.mode}`) : null,
              ),
              h(Select, Object.assign({
                value: selected[key] || "", className: "h-8",
              }, selectChangeHandler(function (v) { pick(key, v); })),
                h(SelectOption, { value: "" }, "Select executor"), executorOptions),
              h("div", { className: "flex flex-wrap gap-1" },
                h(Button, { size: "sm", variant: "outline", disabled: busy || !selected[key],
                  onClick: function () { mutate("/adapters/assign", {
                    shell: key, executor_id: selected[key], responsibility: "candidate",
                    priority: 50, note: "dashboard candidate reinforcement",
                  }, `${key}: candidate strengthened.`); } }, "Strengthen"),
                h(Button, { size: "sm", variant: "outline", disabled: busy || !selected[key],
                  onClick: function () { mutate("/adapters/assign", {
                    shell: key, executor_id: selected[key], responsibility: "primary",
                    note: "dashboard primary assignment",
                  }, `${key}: primary changed.`); } }, "Set primary"),
                h(Button, { size: "sm", variant: "outline", disabled: busy || !selected[key],
                  onClick: function () { mutate("/adapters/switch", {
                    target: key, scope_type: "shell", executor_id: selected[key],
                    mode: "temporary", duration_seconds: 3600,
                    reason: "dashboard role one-hour switch",
                  }, `${key}: switched for one hour.`); } }, "1h"),
                active ? h(Button, { size: "sm", variant: "outline", disabled: busy,
                  onClick: function () { mutate("/adapters/clear", {
                    override_id: active.override_id, reason: "dashboard role restore",
                  }, `${key}: override cleared.`); } }, "Restore") : null,
              ),
            );
          }),
        ) : h("div", { className: "text-xs text-muted-foreground" }, "Loading adapters…"),
        data ? h("details", { className: "border-t pt-2" },
          h("summary", { className: "text-xs font-medium cursor-pointer" },
            `Recent adapter tasks (${(data.recent_tasks || []).length})`),
          h("div", { className: "flex flex-col gap-1 mt-2 max-h-64 overflow-auto" },
            (data.recent_tasks || []).map(function (task) {
              const last = task.last_run || {};
              return h("div", { key: task.task_id, className: "text-[10px] border-b pb-1" },
                h("div", { className: "font-medium" }, `${task.title} · ${task.status}${task.rerunnable ? " · rerunnable" : ""}`),
                h("div", { className: "font-mono text-muted-foreground" },
                  `${task.task_id} · ${task.shell_key || "-"} · ${last.executor_name || last.executor_id || "not run"}${last.runtime ? ` · ${runtimeLabel(last.runtime)}` : ""}`));
            })),
        ) : null,
        data ? h("details", { className: "border-t pt-2" },
          h("summary", { className: "text-xs font-medium cursor-pointer" },
            `Adapter history (${(data.history || []).length})`),
          h("div", { className: "flex flex-col gap-1 mt-2 max-h-64 overflow-auto" },
            (data.history || []).map(function (event) {
              return h("div", { key: event.id, className: "text-[10px] font-mono border-b pb-1" },
                `${event.kind} · ${event.scope_type || "-"}:${event.scope_key || "-"} · ${event.executor_id || "-"} · ${timeAgo ? timeAgo(event.created_at) : event.created_at}`);
            })),
        ) : null,
      ),
    );
  ...
  function TaskDrawer(props) {
    ...
    const doAdapterAction = function (action, payload) {
      let path = "/adapters/switch";
      if (action === "clear") path = "/adapters/clear";
      if (action === "rerun") {
        path = `/tasks/${encodeURIComponent(props.taskId)}/adapter-rerun`;
      }
      return SDK.fetchJSON(withBoard(`${API}${path}`, boardSlug), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      }).then(function (res) {
        load(); props.onRefresh();
        if (action === "rerun" && res && res.revision_task_id) {
          window.alert(`Revision card created: ${res.revision_task_id}`);
        }
        return res;
      }).catch(function (e) {
  ...
  function TaskAdapterSection(props) {
    const info = props.adapter || {};
    const task = props.task;
    const options = (info.available_executors || []).filter(function (e) {
      return e.compatible && e.enabled;
    });
    const [executorId, setExecutorId] = useState("");
    const [busy, setBusy] = useState(false);
    const [msg, setMsg] = useState(null);
    if (!task.role_shell_id) return null;
    const active = info.effective_override;
    const invoke = function (action, payload, text) {
      setBusy(true); setMsg(null);
      return props.onAction(action, payload)
        .then(function () { setMsg({ ok: true, text: text }); })
        .catch(function (e) { setMsg({ ok: false, text: parseApiErrorMessage(e) }); })
        .finally(function () { setBusy(false); });
    };
    return h("div", { className: "hermes-kanban-section" },
      h("div", { className: "hermes-kanban-section-head" }, "Kanban adapter"),
      h("div", { className: "text-xs mb-1" },
        `role: ${(info.task && info.task.shell_key) || task.role_shell_id}`),
      active ? h("div", { className: "text-xs text-amber-600 mb-2" },
        `active override: ${active.executor_id} · ${active.mode} · ${active.override_id}`)
        : h("div", { className: "text-xs text-muted-foreground mb-2" },
          "No active override; primary/candidate routing applies."),
      h("div", { className: "flex flex-wrap gap-2 items-center" },
        h(Select, Object.assign({ value: executorId, className: "h-8 min-w-[220px]" },
          selectChangeHandler(setExecutorId)),
          h(SelectOption, { value: "" }, "Select executor"),
          options.map(function (e) {
            return h(SelectOption, { key: e.executor_id, value: e.executor_id },
              `${e.display_label || `${e.name} · ${runtimeLabel(e)}`} · ${e.responsibility} · p${e.priority}`);
          })),
        task.status === "done" ? h(Button, {
          size: "sm", variant: "outline", disabled: busy || !executorId,
          onClick: function () {
            const reason = window.prompt("Reason for re-requesting this completed card", "re-evaluate with a different adapter");
            if (reason === null) return;
            invoke("rerun", { executor_id: executorId, reason: reason },
              "Revision card created with a one-shot adapter.");
          },
        }, "Re-request revision") : null,
        task.status !== "done" ? h(Button, {
          size: "sm", variant: "outline", disabled: busy || !executorId,
          onClick: function () { invoke("switch", {
            target: task.id, scope_type: "task", executor_id: executorId,
            mode: "once", reason: "dashboard task one-shot switch",
          }, "This card will use the selected adapter once."); },
        }, "Use once") : null,
        task.status !== "done" ? h(Button, {
          size: "sm", variant: "outline", disabled: busy || !executorId,
          onClick: function () { invoke("switch", {
            target: task.id, scope_type: "task", executor_id: executorId,
            mode: "temporary", duration_seconds: 3600,
            reason: "dashboard task one-hour switch",
          }, "This card is switched for one hour."); },
        }, "Use 1h") : null,
        active ? h(Button, {
          size: "sm", variant: "outline", disabled: busy,
          onClick: function () { invoke("clear", {
            override_id: active.override_id, reason: "dashboard task restore",
      ...
      h("div", { className: "text-[10px] text-muted-foreground mt-2" },
        "Every switch, selection, clear, and revision is recorded in Events and Run history."),
    );
  }
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "hermes-kanban-drawer-body" },
      ...
      h(TaskAdapterSection, {
        ...
        onAction: props.onAdapterAction,
      }),
      ...
      visible.map(function (r) {
        ...
        return h("div", { key: r.id, className: cn("hermes-kanban-run", outcomeClass) },
          h("div", { className: "hermes-kanban-run-head" },
            ...
            r.executor_id ? h("span", { className: "hermes-kanban-run-profile" },
              `adapter=${r.executor_id}`) : null,
            r.step_key ? h("span", { className: "hermes-kanban-run-profile" },
              `step=${r.step_key}`) : null,
          ...
          r.error
            ...
            : null,
          r.adapter_override_id
            ? h("div", { className: "text-[10px] text-muted-foreground" },
                `override=${r.adapter_override_id} · binding=${r.binding_id || "-"}`)
            : (r.binding_id
                ? h("div", { className: "text-[10px] text-muted-foreground" },
                    `binding=${r.binding_id}`)
                : null),
