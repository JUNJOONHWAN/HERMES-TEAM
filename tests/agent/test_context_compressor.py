...
def test_supervisor_summary_call_locks_fallback_and_enforces_12k_ceiling():
    response = MagicMock()
    ...
    compressor.locked_fallback_runtimes = locked_chain
    turns = [
    ...
    assert kwargs["enforce_max_tokens"] is True
    assert 0 < kwargs["max_tokens"] <= 12_000
    assert kwargs["locked_fallback_chain"] == locked_chain
