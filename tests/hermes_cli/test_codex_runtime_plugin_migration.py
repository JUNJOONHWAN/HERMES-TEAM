...
class TestMigrate:
    ...
    def test_default_preserves_user_sandbox_mode(self, tmp_path):
        """The default migration must not override the user's sandbox_mode.

        On Linux hosts where bubblewrap cannot create its network namespace,
        injecting :workspace makes every command fail before execution even
        when the user explicitly selected danger-full-access.
        """
