...
def run_conversation(
    ...
    output_cap_adjustment_attempts = 0
    ...
    # Bounded private rewrites for user-visible response contamination.  Keep
    # this independent from API, output-cap, and compression retry budgets.
    response_language_repair_attempts = 0
    supervisor_contract_repair_attempts = 0
    supervisor_contract_failback_attempts = 0
    supervisor_control_tool_called = False
    supervisor_control_tool_names_called: set[str] = set()
    supervisor_repair_delegation_called = False
    supervisor_tooling_delegation_called = False
    supervisor_forced_tool_name = None
    supervisor_operator_text: str | None = None
    ...
    _supervisor_contract_active = supervisor_control_plane_active(agent)
    _supervisor_control_required = bool(
        _supervisor_contract_active
        and supervisor_control_tool_required(original_user_message)
    )
    _supervisor_repair_required = bool(
        _supervisor_contract_active
        and supervisor_repair_delegation_required(original_user_message)
    )
    _supervisor_tooling_required = bool(
        _supervisor_contract_active
        and supervisor_tool_management_delegation_required(original_user_message)
    )
    _supervisor_required_tool = (
        supervisor_required_tool_name(original_user_message)
        if _supervisor_contract_active
        else None
    )
    # Clear control-plane intents are deterministic UI boundaries.  Pin the
    # matching tool on the first request so a provider cannot add a broader
    # status call whose compact operator text hides the requested view.
    supervisor_forced_tool_name = _supervisor_required_tool
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if (
                    _supervisor_control_required
                    and (
                        not supervisor_control_tool_called
                        or (
                            _supervisor_required_tool is not None
                            and _supervisor_required_tool
                            not in supervisor_control_tool_names_called
                        )
                        or (
                            _supervisor_repair_required
                            and not supervisor_repair_delegation_called
                        )
                        or (
                            _supervisor_tooling_required
                            and not supervisor_tooling_delegation_called
                        )
                    )
                ):
                    # The central controller must make a control-plane call on
                    # every substantive operator turn.  ``required`` leaves
                    # the normal routing judgment to the controller model.
                    # Only a bounded same-model recovery below pins a specific
                    # tool after the model has already violated that contract.
                    api_kwargs["tool_choice"] = supervisor_tool_choice_payload(
                        agent.api_mode,
                        supervisor_forced_tool_name,
                    )
                ...
                # A successful response proves the one-shot output-cap recovery
                # converged. Reset its counter independently from compression.
                output_cap_adjustment_attempts = 0
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # The retry path adds diagnostic context before rebuilding
                        # the request. A 64-token margin was one token smaller than
                        # the observed vLLM retry growth (65 tokens), producing an
                        # exact 131,073 > 131,072 loop. Reserve up to 1K instead.
                        reserve = min(1024, max(64, available_out // 16))
                        safe_out = max(1, available_out - reserve)
                        ...
                        # Output-cap correction is not context compression. Keep
                        # an independent bounded retry counter so a recoverable
                        # cap mismatch cannot exhaust the compression budget.
                        output_cap_adjustment_attempts += 1
                        if output_cap_adjustment_attempts > max_compression_attempts:
                            agent._flush_status_buffer()
                            agent._vprint(f"{agent.log_prefix}❌ Output-cap adjustment failed after {max_compression_attempts} retries.", force=True)
                            agent._vprint(f"{agent.log_prefix}   💡 Lower model.max_tokens in config.yaml.", force=True)
                            logger.error(f"{agent.log_prefix}Output-cap adjustment failed after {max_compression_attempts} retries.")
                            agent._persist_session(messages, conversation_history)
                            _final_response = f"Output token cap adjustment failed after {max_compression_attempts} retries."
                            return {
                                "final_response": _final_response,
                                "messages": messages,
                                "completed": False,
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                if getattr(agent, "_supervisor_mode", False):
                    from agent.supervisor_contract import (
                    ...
                    supervisor_control_tool_called = bool(
                        supervisor_control_tool_called
                        or any(
                            tc.function.name in SUPERVISOR_CONTROL_TOOL_NAMES
                            for tc in assistant_message.tool_calls
                        )
                    )
                    supervisor_control_tool_names_called.update(
                        tc.function.name
                        for tc in assistant_message.tool_calls
                        if tc.function.name in SUPERVISOR_CONTROL_TOOL_NAMES
                    )
                    if supervisor_repair_delegation_required(
                        original_user_message
                    ) or supervisor_tool_management_delegation_required(
                        original_user_message
                    ):
                        for tc in assistant_message.tool_calls:
                            if tc.function.name != "supervisor_delegate":
                                continue
                            try:
                                _delegate_args = json.loads(
                                    tc.function.arguments or "{}"
                                )
                            except (TypeError, json.JSONDecodeError):
                                continue
                            if not isinstance(_delegate_args, dict):
                                continue
                            _delegate_args, _eligible_repair_call = (
                                normalize_supervisor_repair_delegation(
                                    original_user_message,
                                    tc.function.name,
                                    _delegate_args,
                            ...
                            tc.function.arguments = json.dumps(
                                _delegate_args, ensure_ascii=False
                            )
                            if _eligible_repair_call:
                                supervisor_repair_delegation_called = True
                            _delegate_args, _eligible_tooling_call = (
                                normalize_supervisor_tool_management_delegation(
                                    original_user_message,
                                    tc.function.name,
                                    _delegate_args,
                            ...
                            if _eligible_tooling_call:
                                supervisor_tooling_delegation_called = True
                ...
                if getattr(agent, "_supervisor_mode", False):
                    _supervisor_call_names = {
                        str(tc.id): tc.function.name
                        for tc in assistant_message.tool_calls
                        if tc.function.name in SUPERVISOR_CONTROL_TOOL_NAMES
                    }
                    if _supervisor_call_names:
                        for _tool_message in reversed(messages):
                            if not isinstance(_tool_message, dict):
                                continue
                            if _tool_message.get("role") != "tool":
                                continue
                            _call_id = str(_tool_message.get("tool_call_id") or "")
                            _call_name = _supervisor_call_names.get(_call_id)
                            if _call_name is None:
                                continue
                            if (
                                _supervisor_required_tool is not None
                                and _call_name != _supervisor_required_tool
                            ):
                                continue
                            _operator_text = (
                                supervisor_operator_text_from_tool_result(
                                    _tool_message.get("content")
                                )
                            )
                            if _operator_text:
                                supervisor_operator_text = _operator_text
                                break
                ...
                # Compact supervisor tools own their screen contract.  The
                # controller model chooses the tool; it does not paraphrase or
                # decorate the deterministic mobile view returned by that tool.
                if _supervisor_contract_active and supervisor_operator_text:
                    final_response = supervisor_operator_text

                if _supervisor_contract_active:
                    _supervisor_contract_satisfied = bool(
                        supervisor_control_tool_called
                        and (
                            _supervisor_required_tool is None
                            or _supervisor_required_tool
                            in supervisor_control_tool_names_called
                        )
                        and (
                            not _supervisor_repair_required
                            or supervisor_repair_delegation_called
                        )
                        and (
                            not _supervisor_tooling_required
                            or supervisor_tooling_delegation_called
                        )
                    )
                    if (
                        _supervisor_control_required
                        and not _supervisor_contract_satisfied
                    ):
                        if supervisor_contract_repair_attempts < 1:
                            supervisor_contract_repair_attempts += 1
                            supervisor_forced_tool_name = (
                                supervisor_recovery_tool_name(
                                    original_user_message
                                )
                            ...
                            logger.warning(
                                "Supervisor controller returned without the required "
                                "control or repair-delegation call; retrying the same "
                                "controller with tool_choice=%s (session=%s)",
                                supervisor_forced_tool_name,
                                getattr(agent, "session_id", None) or "none",
                            )
                            # Keep the first recovery silent and local.  The
                            # rejected draft was never appended to history, so
                            # this preserves role alternation and prompt cache.
                            agent._clear_status_buffer()
                            continue
                        if (
                            supervisor_contract_failback_attempts < 1
                            and activate_codex_control_failback(agent)
                        ):
                            supervisor_contract_failback_attempts += 1
                            supervisor_forced_tool_name = None
                            active_system_prompt = _sync_failover_system_message(
                            ...
                            logger.error(
                                "Supervisor controller contract remained unsatisfied "
                                "after same-controller recovery; failed back directly "
                                "to Codex "
                                "(session=%s)",
                                getattr(agent, "session_id", None) or "none",
                            )
                            agent._buffer_status(
                                "⚠️ 감독 호출 자동 복구가 실패해 이번 턴만 "
                                "Codex로 전환합니다."
                            )
                            continue
                        logger.error(
                            "Supervisor controller contract failed closed: required "
                            "control or repair delegation was absent and Codex failback "
                            "is unavailable (session=%s)",
                            getattr(agent, "session_id", None) or "none",
                        )
                        final_response = (
                            "Hermes 제어 계약 위반으로 응답을 차단했습니다. "
                            "필수 제어 또는 Codex 수선 위임이 실행되지 않았고 "
                            "Codex 컨트롤러 전환도 "
                            "완료되지 않아 어떤 조회·수정·중지도 실행하지 않았습니다."
                        )
                ...
                while (
                    ...
                    and (
                        ...
                        or messages[-1].get("_response_language_repair_synthetic")
                    )
                ):
                    messages.pop()

                # Validate the final prose before verification hooks, durable
                # persistence, or gateway delivery.  A contaminated draft is
                # supplied only as private retry context and both synthetic
                # messages are marked non-persistent.  Telegram streaming can
                # be disabled independently so the draft is not previewed
                # before this final-response gate runs.
                _response_language = getattr(agent, "_response_language", "")
                _language_violation = inspect_response_language(
                    final_response,
                    _response_language,
                )
                if _language_violation is not None:
                    if response_language_repair_attempts < 2:
                        response_language_repair_attempts += 1
                        logger.warning(
                            "Response language guard rejected mixed-script draft "
                            "(language=%s count=%d sample=%r attempt=%d/2 "
                            "session=%s)",
                            _language_violation.language,
                            _language_violation.count,
                            _language_violation.sample,
                            response_language_repair_attempts,
                            getattr(agent, "session_id", None) or "none",
                        )
                        final_msg["content"] = final_response
                        final_msg["finish_reason"] = "response_language_repair"
                        final_msg["_response_language_repair_synthetic"] = True
                        messages.append(final_msg)
                        messages.append({
                            "role": "user",
                            "content": build_response_language_repair_prompt(
                                _response_language
                            ),
                            "_response_language_repair_synthetic": True,
                        })
                        agent._session_messages = messages
                        continue

                    logger.error(
                        "Response language guard failed closed after 2 rewrites "
                        "(language=%s count=%d sample=%r session=%s)",
                        _language_violation.language,
                        _language_violation.count,
                        _language_violation.sample,
                        getattr(agent, "session_id", None) or "none",
                    )
                    final_response = response_language_failure_message(
                        _response_language
                    )
                    final_msg["content"] = final_response
                else:
