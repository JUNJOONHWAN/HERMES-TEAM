...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    #    tokens."
    # When vLLM says ``at least N`` the reported input count is NOT the real
    # tokenized prompt size.  It is only the first token past the allowance
    # implied by the requested output cap (window - requested + 1).  Treating
    # that lower bound as an exact count makes every retry reduce max_tokens by
    # only the safety margin, while vLLM reports a matching larger lower bound;
    # the total therefore remains exactly window + 1 until retries exhaust.
    # Use a geometric reduction for this intentionally lossy form so recovery
    # converges even though the provider withheld the exact input size.
    #
    # If vLLM reports an exact input count (no ``at least``), keep returning the
    # exact remaining output budget.  When the input alone is at or over the
    # window this stays None, so the caller correctly falls through to
    # compression instead of futilely shrinking the output cap.
    _m_vllm_input = re.search(
        r'prompt contains (at least\s+)?(\d+)\s*input tokens', error_lower
    )
    if _m_ctx_tok and _m_vllm_input:
        _available = int(_m_ctx_tok.group(1)) - int(_m_vllm_input.group(2))
        if _available >= 1:
            if _m_vllm_input.group(1):
                _m_requested = re.search(
                    r'requested\s+(\d+)\s*output tokens', error_lower
                )
                if _m_requested:
                    _requested = int(_m_requested.group(1))
                    if _requested >= 1:
                        return min(_available, max(1, _requested // 2))
            return _available
