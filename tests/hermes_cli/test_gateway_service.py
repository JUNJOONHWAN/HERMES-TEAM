...
class TestSystemdServiceRefresh:
    ...
    def _mock_user_systemd_preflight(self, monkeypatch):
        """These are systemd unit-flow tests, independent of the host OS."""
...
class TestGeneratedSystemdUnits:
    def _expected_timeout_stop_sec(self) -> str:
        timeout = int(
            max(max(60, DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT) + 30, 3600 + 300)
        )
    ...
    def test_user_unit_avoids_recursive_execstop_and_uses_extended_stop_timeout(self, monkeypatch):
        ...
        # TimeoutStopSec must cover the one-hour cron script timeout plus
        # delivery/teardown headroom before systemd escalates to SIGKILL.
...
class TestGatewaySystemServiceRouting:
    @pytest.fixture(autouse=True)
    def _mock_user_systemd_preflight(self, monkeypatch):
        """Exercise restart routing without requiring Linux D-Bus on macOS CI."""
        monkeypatch.setattr(gateway_cli, "_preflight_user_systemd", lambda: None)
    ...
    def test_systemd_restart_gracefully_restarts_running_service_and_waits(self, monkeypatch, capsys):
        ...
        # Restart=always owns the replacement after the old gateway exits.
        ...
        assert ("graceful", 654, 3905) in calls
    ...
    def test_systemd_restart_uses_systemd_main_pid_when_pid_file_is_missing(self, monkeypatch, capsys):
        ...
        assert ("graceful", 777, 3905) in calls
        assert ("wait", False, 777) in calls
        assert "restarting gracefully (pid 777)" in capsys.readouterr().out.lower()

    def test_systemd_restart_accepts_replacement_when_old_pid_lingers(self, monkeypatch, capsys):
        calls = []
        ...
        monkeypatch.setattr(
            gateway_cli,
            "_run_systemctl",
            lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("must not force restart")),
        )

        gateway_cli.systemd_restart()

        assert calls == [{"system": False, "previous_pid": 654, "timeout": 10.0}]
        assert "forcing a service restart" not in capsys.readouterr().out.lower()
