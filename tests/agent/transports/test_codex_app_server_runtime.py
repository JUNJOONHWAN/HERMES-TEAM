...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_preserves_explicit_codex_full_access(
        self, monkeypatch, tmp_path
    ):
        """An explicit Codex full-access config must not be downgraded.

        Hardened Linux hosts can deny the user namespaces used by the bwrap
        workspace sandbox. The operator's explicit full-access choice is the
        audited opt-in that keeps Kanban shell commands usable on those hosts.
        """
        import subprocess
        from agent.transports import codex_app_server as cas
        ...
        monkeypatch.setenv(
            "HERMES_KANBAN_DB",
            str(tmp_path / ".hermes" / "kanban" / "kanban.db"),
        )
...
def test_market_role_uses_public_sources_without_private_preflight():
    from hermes_cli.supervisor_bootstrap import ROLE_SPECS

    instructions = ROLE_SPECS["market"]["instructions"]
    assert "Yahoo Finance" in instructions
    assert "Naver Finance" in instructions
    assert "Never invent missing values" in instructions
    assert "trade" in instructions
