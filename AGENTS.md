...
## HERMES-TEAM Distribution Contract

This checkout is the distributable HERMES-TEAM project. Before operating or
changing its supervisor, read `AI_OPERATIONS_MANUAL.md` and
`HERMES_PUBLIC_ARCHITECTURE_REPORT_KO.md` in full.

Public-edition invariants:

- OpenCode is the default controller path; its live free-model catalog is
  discovered at setup time. Codex, Grok, OpenRouter, and OpenAI-compatible
  engines are optional health-gated adapters, not hidden defaults.
- The controller delegates domain work to immutable Role Shells through
  Bindings and audited Overrides. Do not bypass the Kanban card or Receipt.
- Every bundled shell requires Timeline and NeuralLink recall evidence. The
  code shell additionally requires a stored Code Map slice. Completion also
  requires Timeline `verify_all.invalid_count == 0` and declared outputs.
- Heartbeat has exactly three canonical layers: `configuration`,
  `service_schedule`, and `artifacts`. Do not reintroduce owner-specific lanes.
- The distribution contains no private database, absolute operator path,
  assumed service, or assumed schedule. Optional domain tools and manually
  accumulated memory are explicit adapters and operator-owned state.
- External capability is attached only through the documented generic adapter
  contract.

Maintainer source-of-truth boundary:

- The canonical HERMES-TEAM release checkout lives on the DGX Spark maintainer
  host. Mac checkouts are temporary mirrors and must not originate releases.
- The public distribution checkout and the maintainer's live Hermes runtime are
  separate repositories. Public release work must never modify, restart, or
  reconfigure the live runtime checkout or its services.
