...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_portable_mktemp_not_shell_pid(self):
        """The temp name must be unique on macOS Bash 3.2 as well as Linux.

        Bash 3.2 does not define ``$BASHPID`` and ``$$`` is shared by
        ``&``-launched subshells, so both PID schemes can collapse to one file.
        ``mktemp`` is portable across the supported POSIX shells.
        """
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
