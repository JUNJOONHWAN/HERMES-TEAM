...
def _apply_supervisor_operator_screen(
    ...
    *,
    ...
    gate.  Normalize the projected tool envelope here so Codex cannot expand a
    compact status or adapter screen back into internal registry prose.
    """
...
def _dispatcher_mcp_elicitation_allowlist() -> frozenset[str]:
    """Return MCP server names trusted for this bound role-shell process.

    The values are not model input or profile defaults. ``executor_adapter``
    stamps them into the worker process from the immutable shell/executor
    capability intersection. Central Hermes and ordinary interactive sessions
    have no role-shell stamp and therefore receive an empty allowlist.
    """
    if not str(os.environ.get("HERMES_ROLE_SHELL_ID") or "").strip():
        return frozenset()
    if not str(os.environ.get("HERMES_KANBAN_TASK") or "").strip():
        return frozenset()
    raw = str(os.environ.get("HERMES_EFFECTIVE_CAPABILITIES") or "")
    return frozenset(
        item.strip()
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        from agent.supervisor_codex_bridge import build_supervisor_codex_context

        cwd = getattr(agent, "session_cwd", None) or str(resolve_agent_cwd())
        supervisor_context = build_supervisor_codex_context(agent)
        if supervisor_context is not None:
            cwd = supervisor_context.cwd
        ...
        def _on_codex_event(note: dict) -> None:
            # Every real app-server notification is agent activity, even when
            # it is not a tool-shaped item or no progress UI is installed.
            # Keep this ahead of the progress callback's early returns so the
            # gateway idle watchdog does not interrupt an active Codex turn.
            event_method = (
                str(note.get("method") or "notification")
                if isinstance(note, dict)
                else "notification"
            )
            try:
                agent._touch_activity(f"codex app-server event: {event_method}")
            except Exception:
        ...
        agent._codex_session = CodexAppServerSession(
            cwd=cwd,
            codex_home=(
                supervisor_context.codex_home if supervisor_context else None
            ),
            developer_instructions=(
                supervisor_context.developer_instructions
                if supervisor_context else None
            ),
            dynamic_tools=(
                supervisor_context.dynamic_tools if supervisor_context else None
            ),
            dynamic_tool_handler=(
                supervisor_context.dynamic_tool_handler
                if supervisor_context else None
            ),
            ...
            request_routing=_ServerRequestRouting(
                ...
                auto_accept_mcp_elicitation_servers=(
                    _dispatcher_mcp_elicitation_allowlist()
                ),
            ),
