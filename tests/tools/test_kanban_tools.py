...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    expected = {
        ...
        "kanban_capability_missing", "kanban_comment", "kanban_create", "kanban_link",
    }
...
def test_capability_missing_routes_to_multitool_and_auto_resumes(worker_env):
    from hermes_cli import kanban_db as kb
    ...
    try:
        shell = sr.register_shell_version(
            conn,
            shell_key="tool-management",
            name="Multitool Management",
            contract={
                "allowed_adapters": ["hermes_profile"],
                "instructions": "Manage role-scoped tools only.",
            },
            required_capabilities=("file", "kanban"),
            allowed_capabilities=("file", "terminal", "web", "skills", "kanban"),
            evidence_policy={"timeline_required": True, "code_slice_required": False},
        )
        executor = sr.register_executor(
            conn,
            executor_id="executor_multitool",
            name="hermes-worker-multitool",
            adapter_type="hermes_profile",
            launch_config={"profile": "hermes-worker-multitool"},
            capabilities=("file", "terminal", "web", "skills", "kanban"),
    ...
    result = json.loads(
        kt._handle_capability_missing(
            {
                "capability": "로그인형 포털 고급 브라우징",
                "details": "required browser tool is absent from this session",
            }
        )
    )
    ...
    assert result["resume_mode"] == "fresh_worker_session"
    assert any(
        row["name"] in {"web", "browser", "example-browser"}
        for row in result["catalog_candidates"]
    )
    dependency_id = result["dependency_task_id"]
    ...
    try:
        ...
        assert dependency.role_shell_id == shell.id
        assert kb.parent_ids(conn, worker_env) == [dependency_id]
        assert kb.list_notify_subs(conn, dependency_id)[0]["chat_id"] == "chat-1"
        assert any(
            event.kind == "capability_missing"
            and event.payload["dependency_task_id"] == dependency_id
            for event in kb.list_events(conn, worker_env)
        )

        assert kb.claim_task(conn, dependency_id) is not None
        run = kb.latest_run(conn, dependency_id)
        receipt = {
            "run_id": run.id,
            ...
            "outputs": [{"kind": "tool_probe", "value": "passed"}],
            "timeline": {
                "goal_id": sr.timeline_goal_id(dependency_id, run.id),
                "context_loaded": True,
                "node_ids": ["node-tool-1"],
                "verify_all": {"invalid_count": 0, "verified_count": 1},
            },
        }
        assert kb.complete_task(
            conn,
            dependency_id,
            summary="tool assignment validated",
            expected_run_id=run.id,
            receipt=receipt,
        )
        # complete_task runs the dependency promotion pass immediately.
