...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # OpenCode Zen's free endpoint is intentionally anonymous.  The OpenAI SDK
    # requires a non-empty api_key and would otherwise send the placeholder as
    # ``Authorization: Bearer no-key-required``; the main-agent transport strips
    # it, but the auxiliary/compression transport historically did not, causing
    # the live 401 -> surprise OpenRouter fallback incident.
    if api_key == "no-key-required" and base_url_host_matches(base_url, "opencode.ai"):
        ...
        if isinstance(event_hooks, dict):
            ...
            def _strip_anonymous_auth(request: Any) -> None:
                request.headers.pop("Authorization", None)

            request_hooks.append(_strip_anonymous_auth)
        headers = dict(kwargs.get("default_headers") or {})
        headers.setdefault("User-Agent", "HermesAgent/opencode-zen-free")
...
def resolve_provider_client(
    ...
    # ``auto`` owns provider+model selection as one runtime pair inside
    # ``_resolve_auto``.  Pulling the persisted main model here can combine a
    # request-scoped controller provider (for example OpenCode HY3) with the
    # configured fallback model (Codex), producing provider=opencode-zen with
    # model=gpt-5.6-sol.  Defer both axes for auto; _resolve_auto already reads
    # the live main_runtime first and the configured runtime second.
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if enforce_max_tokens:
            if _provider_norm in {"openai-codex", "codex"}:
...
def call_llm(
    ...
    *,
    ...
    enforce_max_tokens: bool = False,
    locked_fallback_chain: Optional[List[Dict[str, Any]]] = None,
) -> Any:
    ...
    try:
        ...
        if should_fallback and locked_fallback_chain is not None:
            # A controller-owned auxiliary task must stay inside the audited
            # controller chain.  In particular, a HY3 compression failure may
            # use its configured Codex failback, but it must never wander into
            # ambient OpenRouter credentials discovered by the generic auto
            # chain.  An empty list means fail closed on the current provider.
            last_locked_error: Optional[BaseException] = None
            for entry in locked_fallback_chain:
                if not isinstance(entry, dict):
                    continue
                fb_provider = str(entry.get("provider") or "").strip()
                fb_requested_model = str(entry.get("model") or "").strip()
                if not fb_provider or not fb_requested_model:
                    continue
                try:
                    fb_client, fb_model = _get_cached_client(
                        fb_provider,
                        fb_requested_model,
                    ...
                    fb_kwargs = _build_call_kwargs(
                        fb_provider,
                        fb_model or fb_requested_model,
                        messages,
                        temperature=temperature,
                        max_tokens=max_tokens,
                        tools=tools,
                    ...
                    logger.info(
                        "Auxiliary %s: using locked controller fallback %s (%s)",
                        task or "call", fb_provider, fb_model or fb_requested_model,
                    )
                    return _validate_llm_response(
                        fb_client.chat.completions.create(**fb_kwargs), task
                    )
                except Exception as locked_error:
                    last_locked_error = locked_error
                    logger.warning(
                        "Auxiliary %s: locked controller fallback %s failed: %s",
                        task or "call", fb_provider, locked_error,
                    )
