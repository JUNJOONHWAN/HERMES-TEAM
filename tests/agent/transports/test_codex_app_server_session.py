...
class TestLifecycle:
    ...
    def test_thread_start_passes_cwd_without_permissions(self):
    ...
    def test_thread_start_passes_developer_identity_and_dynamic_tools(self):
        client = FakeClient()
        dynamic_tools = [
            {
                "type": "function",
        ...
        s = make_session(
            client,
            developer_instructions="You are the Hermes controller.",
            dynamic_tools=dynamic_tools,
        )
        s.ensure_started()
        _method, params = next(
            row for row in client.requests if row[0] == "thread/start"
        )
        assert params["developerInstructions"] == "You are the Hermes controller."
...
class TestServerRequestRouting:
    def test_dynamic_tool_call_uses_local_handler(self):
        client = FakeClient()
        client.queue_server_request(
            "item/tool/call",
            request_id="dyn-1",
            tool="supervisor_adapter",
        ...
        def handler(name, args):
            captured["name"] = name
            captured["args"] = args
            return True, '{"control_slot_count": 7}'

        s = make_session(client, dynamic_tool_handler=handler)
        s.run_turn("현재 어댑터 현황", turn_timeout=1.0)
        ...
        assert (
            ...
            {
                "contentItems": [
                    {"type": "inputText", "text": '{"control_slot_count": 7}'}
                ],
                "success": True,
            },
        ) in client.responses

    def test_dynamic_tool_call_fails_closed_without_handler(self):
        client = FakeClient()
        client.queue_server_request(
            "item/tool/call",
            request_id="dyn-2",
        ...
        s = make_session(client)
        s.run_turn("status", turn_timeout=1.0)
        response = next(result for rid, result in client.responses if rid == "dyn-2")
        assert response["success"] is False
        assert "No Hermes handler" in response["contentItems"][0]["text"]
    ...
    def test_mcp_elicitation_for_dispatcher_allowed_worker_server_accepts(self):
        """A bound worker may use only MCP servers granted by its effective
        role-shell capability intersection; unrelated servers still decline."""
        client = FakeClient()
        client.queue_server_request(
            "mcpServer/elicitation/request", request_id="elic-worker",
            threadId="t", turnId="tu1",
            serverName="hermes-timeline-code-map",
            mode="form", message="confirm",
            requestedSchema={"type": "object", "properties": {}},
        )
        client.queue_server_request(
            "mcpServer/elicitation/request", request_id="elic-unrelated",
            threadId="t", turnId="tu1",
            serverName="profile-only-server",
            mode="form", message="confirm",
            requestedSchema={"type": "object", "properties": {}},
        )
        client.queue_notification(
        ...
        s = make_session(
            ...
            request_routing=_ServerRequestRouting(
                auto_accept_mcp_elicitation_servers=frozenset(
