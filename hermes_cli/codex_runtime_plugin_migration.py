...
def migrate(
    ...
    *,
    ...
    Args:
        ...
        default_permission_profile: when set, write
            top-level `default_permissions = "<name>"` so users on this
            runtime don't get an approval prompt on every write attempt.
            Known built-in Codex 0.144.x profile names are ":workspace" and
            ":read-only" (note the leading ":"). Also accepts a user-defined
            profile name (no leading ":") configured in the user's own
            [permissions.<name>] table. The default is None so an explicit
            top-level sandbox_mode such as "danger-full-access" remains
            authoritative instead of being silently overridden.
