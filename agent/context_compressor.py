...
class ContextCompressor(ContextEngine):
    ...
    def __init__(
        ...
        self.provider = provider
        self.api_mode = api_mode
        # ``None`` keeps the normal global auxiliary discovery policy.  The
        # Hermes supervisor sets this to its audited controller fallback chain
        # so compression can use only controller -> Codex, never a surprise
        # OpenRouter/provider discovered from ambient credentials.  Model
        # switches preserve this policy; it belongs to the controller, not the
        # selected model.
        self.locked_fallback_runtimes: Optional[List[Dict[str, Any]]] = None
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
        ...
        try:
            call_kwargs = {
                ...
                # Preserve headroom without ever exceeding the compressor's
                # own 12K summary ceiling.  Compatible providers otherwise use
                # their model default (65,536 on the incident route), which can
                # turn a harmless compaction into a billing/credit failure.
                "max_tokens": min(
                    _SUMMARY_TOKENS_CEILING,
                    max(1, int(summary_budget * 1.3)),
                ),
                "enforce_max_tokens": True,
                # timeout resolved from auxiliary.compression.timeout config by call_llm
            }
            locked_fallback_runtimes = getattr(
                self, "locked_fallback_runtimes", None
            )
            if locked_fallback_runtimes is not None:
                call_kwargs["locked_fallback_chain"] = list(
                    locked_fallback_runtimes
                )
