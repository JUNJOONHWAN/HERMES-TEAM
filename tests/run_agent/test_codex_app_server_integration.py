...
class TestRunConversationCodexPath:
    ...
    def test_supervisor_operator_screen_is_verbatim_on_codex_path(self, monkeypatch):
        operator_text = (
            "Hermes 정상\n"
            "서비스 7/7\n"
            "자동화 18 · 활성 16 · 일시정지 2 · 실패 0\n"
            "역할 7/7"
        )
            ...
            return TurnResult(
                ...
                projected_messages=[
                    ...
                    },
                    {
                        ...
                        "content": json.dumps(
                            [{"type": "inputText", "text": payload}],
                            ensure_ascii=False,
                        ),
                    },
                    {
                        "role": "assistant",
                        "content": "모델이 임의로 늘린 상태 설명",
                    },
                ],
                tool_iterations=1,
        ...
        monkeypatch.setattr(
            ...
            "ensure_started",
            lambda self: "thread-status-1",
        )
        agent = _make_codex_agent()
        agent._supervisor_mode = True

        result = agent.run_conversation("전체 상태 알려줘")
    ...
    def test_manual_approvals_keep_codex_server_requests_fail_closed(
        ...
        assert routing.auto_accept_mcp_elicitation_servers == frozenset()

    def test_bound_role_worker_accepts_only_dispatcher_capability_mcp_servers(
        self, monkeypatch
    ):
        captured = self._capture_routing_agent(monkeypatch)
        monkeypatch.setenv("HERMES_ROLE_SHELL_ID", "shell_verification_v1")
        monkeypatch.setenv("HERMES_KANBAN_TASK", "t_worker")
        monkeypatch.setenv(
            "HERMES_EFFECTIVE_CAPABILITIES",
            "kanban,hermes-timeline-code-map,example-market-data",
        )
        with patch(
            "hermes_cli.config.load_config",
            return_value={"approvals": {"mode": "manual"}},
            ...
            with patch.object(
                ...
                agent.run_conversation("verify result")
        routing = captured["request_routing"]
        assert routing.auto_accept_mcp_elicitation_servers == frozenset(
            {"kanban", "hermes-timeline-code-map", "example-market-data"}
        )
...
class TestCodexToolProgressBridge:
    ...
    def test_session_event_touches_activity_without_progress_callback(self, monkeypatch):
        """Every app-server notification refreshes the gateway heartbeat,
        including non-tool events when no progress callback is installed."""
        captured_init = {}
        ...
        def fake_run_turn(self, user_input, **kwargs):
            ...
            if on_event:
                on_event({"method": "item/completed", "params": {"item": {
                    "type": "agentMessage", "text": "still working"}}})
            return TurnResult(final_text="done", projected_messages=[
                {"role": "assistant", "content": "done"}], turn_id="t1", thread_id="th1")
