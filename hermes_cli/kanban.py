...
def _fmt_task_line(t: kb.Task) -> str:
    ...
    assignee = t.role_shell_id or t.assignee or "(unassigned)"
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_create.add_argument(
        ...
        default=None,
        help=(
            "Immutable active role-shell version id. The supervisor selects a "
            "real executor through its many-to-many binding registry."
        ),
    )
    ...
    p_complete.add_argument(
        "--result", default=None, help="Exact complete user-facing result"
    )
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        task_id = kb.create_task(
            ...
            role_shell_id=getattr(args, "role_shell_id", None),
        )
        task = kb.get_task(conn, task_id)
    if getattr(args, "json", False):
        print(json.dumps(_task_to_dict(task), indent=2, ensure_ascii=False))
    else:
        owner = task.role_shell_id or task.assignee or "-"
        print(f"Created {task_id}  ({task.status}, owner={owner})")
        ...
        if task.status == "ready" and (task.assignee or task.role_shell_id):
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        ...
        # ``tasks.result`` is the canonical deliverable for new supervisor-bound
        # work. Keep the latest run summary beside it for legacy/unbound cards
        # and for the intentionally short handoff shown in Run History.
    ...
    print(f"  role shell:{' ' if task.role_shell_id else ''}{task.role_shell_id or '-'}")
    if task.tenant:
