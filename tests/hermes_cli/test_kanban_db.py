...
def test_dispatch_cancels_worker_spawned_after_task_was_archived(
    kanban_home, all_assignees_spawnable, monkeypatch,
):
    """Archive winning the spawn race must not leave an orphan process."""
    terminated = []
    ...
    with kb.connect() as conn:
        tid = kb.create_task(conn, title="archive during spawn", assignee="alice")

        def spawn_then_archive(task, _workspace):
            assert kb.archive_task(conn, task.id) is True
            return 424242

        result = kb.dispatch_once(conn, spawn_fn=spawn_then_archive)
        task = kb.get_task(conn, tid)
    ...
    assert terminated[0][0] == 424242
    assert terminated[0][1]
    assert "spawned" not in event_kinds
    assert "spawn_cancelled_terminal_race" in event_kinds


def test_archive_running_task_terminates_worker_and_closes_run(
    kanban_home, monkeypatch,
):
    signals = []
    monkeypatch.setattr(kb, "_pid_alive", lambda _pid: False)
    ...
    with kb.connect() as conn:
        tid = kb.create_task(conn, title="archive running", assignee="worker")
    ...
    assert task.status == "archived"
    assert task.worker_pid is None
    assert signals and signals[0][0] == 515151
...
def test_worker_termination_signals_spawned_process_group(monkeypatch):
    ...
    assert signals and signals[0][0] == 616161
    assert info["host_local"] is True
    assert info["termination_attempted"] is True


def test_archived_worker_survivor_is_retained_and_reaped_next_tick(
    kanban_home, all_assignees_spawnable, monkeypatch,
):
    attempts = []

    def fake_terminate(pid, claim_lock, **_kwargs):
        attempts.append((pid, claim_lock))
        terminated = len(attempts) >= 2
        return {
            "prev_pid": pid,
            "host_local": True,
            "termination_attempted": True,
    ...
    with kb.connect() as conn:
        tid = kb.create_task(conn, title="stubborn worker", assignee="worker")
        claimed = kb.claim_task(conn, tid)
        assert claimed is not None
        assert kb._set_worker_pid(conn, tid, 717171)

        assert kb.archive_task(conn, tid)
        assert kb.get_task(conn, tid).worker_pid == 717171
    ...
    assert "archive_worker_reaped" in event_kinds
