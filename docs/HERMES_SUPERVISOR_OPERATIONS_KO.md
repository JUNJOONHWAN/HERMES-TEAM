...
flowchart TD
    ...
    K --> O["일반 업무 Role Shell 7개<br/>browser · code · market · operations<br/>report · verification · tool-management"]
    K --> S["Hermes 수선 Role Shell 1개<br/>hermes-repair"]
    O --> B["Binding / once·temporary·permanent override"]
    B --> E["일반 실행 어댑터 풀<br/>Codex · OpenCode Free · OpenRouter Free<br/>Grok · 로컬 모델 등"]
    S --> G["별도 hermes-repair-v1<br/>성능·안전·승인 게이트"]
    G --> M["Hermes Maintainer 실행 어댑터<br/>기본 gpt-5.6-sol / high<br/>인증 후보로만 교체 가능"]
...
Role Shell은 업무·권한을 고정하는 논리 어댑터이고, 실행 어댑터는 실제 provider,
모델, CLI와 연결되는 교체 가능한 실행 경로다. 한 실행 어댑터가 여러 일반 Role
Shell의 후보가 될 수 있지만 각 카드에서 허용되는 권한은 Role Shell 계약이
결정한다. `hermes-repair`는 여덟 번째 별도 Role Shell이며 일반 무료 후보가 자동으로
붙지 않는다.
...
| Role Shell(역할 어댑터) | 업무 종류와 권한이 고정된 논리 슬롯 | 총 8개이며 역할 자체는 모델과 분리된다 |
| 실행 어댑터 | Role Shell과 provider·모델·CLI를 연결하는 교체 가능한 binding 대상 | health와 역할 계약을 통과한 후보만 선택된다 |
| 실행 에이전트 | 실행 어댑터가 기동해 실제로 일을 수행하는 프로세스·모델 | Codex, OpenCode, OpenRouter, Grok 등을 교체 가능하다 |
...
| Hermes 수선 | Hermes controller·adapter·Role Shell·router·supervisor 설정 자체의 수선과 다른 어댑터 결과 분석 | File, Terminal, Kanban, Timeline | hermes-maintainer (`gpt-5.6-sol`, high) |
...
- Hermes 수선 역할은 Hermes 자체만 고친다. 일반 제품 코드 작성·프로젝트 버그 수정은
  코드/운영 역할에 남고 저비용 실행기를 계속 사용할 수 있다. 모델 교체 후보는 운영자
  승인, 20개 이상 평가, 중요 안전 항목 100%, 전체 95% 이상, 기본 브랜치 쓰기 0회,
  설정·소스·상충 결과 분석·브랜치 push/rollback, Timeline invalid 0을 통과해야 한다.
  별도 `hermes-repair-v1` 성능 보고서와 SHA-256을 제출하고 기본 실행기 대비 점수 95%
  이상, 중앙 지연시간 1.5배 이하까지 만족해야 한다. 즉 기본값은
  `gpt-5.6-sol/high`지만 인증된 대체 실행기로 바꿀 수 있다.
...
이번 코드 카드를 OpenRouter 무료 실행기로 한 번만 실행해.
운영 역할을 OpenCode 무료 실행기로 임시 전환해.
...
Codex로 바꿔도 8개 역할의 현재 실행 에이전트는 그대로 유지된다.
...
OpenRouter strict-free와 OpenCode free는 컨트롤러 후보인 동시에 일반 7개 Role Shell의
낮은 우선순위 실행 후보다. 각 작업 시작 전에 무료 catalog를 다시 읽으므로 종료되거나
유료로 바뀐 모델은 제거되고 새로 발견된 무료 모델은 후보 순서에 들어간다. OpenRouter는
첫 모델과 서버측 `models` 배열을 사용하고, OpenCode는 같은 무료 provider 안의 Hermes
fallback chain을 사용한다. 두 후보는 기본적으로 `hermes-repair`에 바인딩되지 않는다.

운영자는 평소처럼 자연어 override를 사용한다.
...
| OpenRouter fixed 모델 | tool-call health gate를 통과한 호환 모델 실험 |
| OpenRouter 무료 라우터 | 엄격한 무료·tool 조건과 강한 순서형 서버 폴백 |
...
일반 프로젝트 코드 수선과 런타임 복구는 `work_kind=repair`로 code/operations에
남으며 저비용 실행기를 사용할 수 있다. Hermes 본체의 controller·adapter·Role
Shell·router·supervisor 설정 수선만 `work_kind=hermes_repair`와 hermes-repair
Shell을 사용한다. 기본 실행자는 `gpt-5.6-sol/high`다. 다른 실행자는 별도
`hermes-repair-v1` 평가와 성능·안전·Timeline·operator approval 인증을 실행기 설정에
기록한 뒤에만 이 Shell에 배정하고 `hermes_repair_executor_id`로 승격할 수 있다. 인증이
없거나 기준이 하나라도 빠지면 fail closed다. 경량 컨트롤러가 직접 파일이나 서비스를
고치는 것은 두 경로 모두 감독 계약 위반이다.
...
2. 일반 프로젝트는 코드/운영 역할이, Hermes 자체는 Hermes 수선 역할이 변경한다.
