...
from agent.agent_init import (
    ...
    _merge_model_temperature_request_override,
)
...
def test_model_temperature_config_adds_request_override():
    agent = SimpleNamespace(request_overrides={"service_tier": "priority"})

    _merge_model_temperature_request_override(agent, {"temperature": "0.1"})
...
def test_model_temperature_config_preserves_caller_override():
    agent = SimpleNamespace(request_overrides={"temperature": 0.7})

    _merge_model_temperature_request_override(agent, {"temperature": 0.1})

    assert agent.request_overrides == {"temperature": 0.7}


@pytest.mark.parametrize("value", [True, -0.1, 2.1, "nan", "not-a-number"])
def test_model_temperature_config_rejects_invalid_values(value):
    agent = SimpleNamespace(request_overrides={})

    with pytest.raises((TypeError, ValueError)):
        _merge_model_temperature_request_override(agent, {"temperature": value})
