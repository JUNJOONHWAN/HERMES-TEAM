...
def _handle_capability_missing(args: dict, **kw) -> str:
    """Create a Multitool dependency and park this worker until it completes."""
    tid = _default_task_id(args.get("task_id"))
    if not tid:
        return tool_error(
            "task_id is required (or set HERMES_KANBAN_TASK in the env)"
    ...
    if ownership_err:
        return ownership_err
    capability = redact_sensitive_text(
        str(args.get("capability") or "").strip(), force=True
    )[:240]
    if not capability:
        return tool_error("capability is required")
    details = redact_sensitive_text(
        str(args.get("details") or "").strip(), force=True
    )[:2000]
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
            from hermes_cli import supervisor_registry as supervisor_registry
            ...
            supervisor_registry.ensure_schema(conn)
            source = kb.get_task(conn, tid)
            if source is None:
                return tool_error(f"unknown task: {tid}")
            shell = supervisor_registry.get_shell(
                conn, shell_key="tool-management"
            )
            if shell is None:
                return tool_error(
                    "tool-management role shell is not installed; "
                    "ask the Hermes controller to run supervisor install"
                )
            source_profile = str(
                os.environ.get("HERMES_PROFILE") or source.assignee or "unknown"
            ).strip()
            source_executor = str(
                os.environ.get("HERMES_EXECUTOR_ID") or "unknown"
            ).strip()
            catalog = build_tool_catalog(
                get_default_hermes_root(), conn=conn
            )
            catalog_candidates = search_tool_catalog(catalog, capability)
            idempotency_key = (
                f"capability-missing:{tid}:"
                + hashlib.sha256(
                    capability.casefold().encode("utf-8")
                ).hexdigest()[:16]
            )
            body_lines = [
                f"Source task: {tid}",
                f"Requesting profile: {source_profile}",
                f"Requesting executor: {source_executor}",
                f"Missing capability: {capability}",
                "Required outcome: inspect the central catalog, assign the minimum "
                "role-scoped tool/MCP/skill set, validate it in a fresh target "
                "profile session, and record rollback evidence.",
                "If source/service/secret repair is required, create a code or "
                "operations repair handoff instead of widening Multitool authority.",
            ]
            if details:
                body_lines.append(f"Failure evidence: {details}")
            if catalog_candidates:
                body_lines.append(
                    "Central catalog candidates: "
                    + "; ".join(
                        f"{row['kind']}:{row['name']}"
                        f"@{','.join(row['owners']) or 'global'}"
                        for row in catalog_candidates
                    )
                )
            else:
                body_lines.append(
                    "Central catalog candidates: none; perform public tool discovery."
                )
            dependency_id = kb.create_task(
                conn,
                title=f"도구 보강: {capability}",
                body="\n".join(body_lines),
                role_shell_id=shell.id,
                workspace_kind="scratch",
                priority=max(1, int(source.priority or 0)),
                idempotency_key=idempotency_key,
                session_id=source.session_id,
                created_by=source_profile,
            )
            kb.link_tasks(conn, parent_id=dependency_id, child_id=tid)
            for subscription in kb.list_notify_subs(conn, tid):
                kb.add_notify_sub(
                    conn,
                    task_id=dependency_id,
                    platform=subscription["platform"],
                    chat_id=subscription["chat_id"],
                    thread_id=subscription.get("thread_id") or None,
                    user_id=subscription.get("user_id") or None,
                    notifier_profile=subscription.get("notifier_profile") or None,
                    chat_type=subscription.get("chat_type") or None,
                )
            reason = (
                f"capability_missing: {capability}; waiting for "
                f"tool-management card {dependency_id}"
            )
            ok = kb.block_task(
                conn,
                tid,
            ...
            if not ok:
                return tool_error(
                    f"could not park {tid}; dependency card {dependency_id} "
                    "was preserved for audit"
                )
            ...
            # An idempotent retry can find a dependency that already completed.
            # Recompute immediately so the source does not wait for the next
            # dispatcher heartbeat in that case.
            kb.recompute_ready(conn)
            landed = kb.get_task(conn, tid)
            return _ok(
                task_id=tid,
                dependency_task_id=dependency_id,
                dependency_shell="tool-management",
                catalog_candidates=catalog_candidates,
                status=landed.status if landed else "todo",
                auto_resume=True,
                resume_mode="fresh_worker_session",
            )
        finally:
            conn.close()
    except ValueError as e:
        return tool_error(f"kanban_capability_missing: {e}")
    except Exception as e:
        logger.exception("kanban_capability_missing failed")
        return tool_error(f"kanban_capability_missing: {e}")
...
def _handle_create(args: dict, **kw) -> str:
    ...
    assignee = args.get("assignee")
    role_shell_id = args.get("role_shell_id")
    if not assignee and not role_shell_id:
        return tool_error(
            "assignee or role_shell_id is required — use role_shell_id for "
            "supervisor-routed work and assignee only for legacy profile routing"
        )
    body = args.get("body")
    ...
    try:
        ...
        try:
            ...
            new_tid = kb.create_task(
                ...
                role_shell_id=str(role_shell_id) if role_shell_id else None,
...
def _receipt_schema_prop() -> dict:
    return {
        "type": "object",
        "description": (
            "Required for supervisor-bound role runs. Include outputs and a "
            "timeline object with goal_id, context_loaded=true, slice_ids, "
            "node_ids, neural_recall evidence when required, and "
            "verify_all.invalid_count=0. Identity fields are "
            "stamped and checked against the trusted run. timeline.goal_id is "
            "also dispatcher-stamped when omitted; if supplied it must match. "
            "An executor cannot self-exempt from the role shell's evidence policy."
        ),
        "properties": {
            ...
            "timeline": {
                ...
                "properties": {
                    ...
                    "verify_all": {
                        "type": "object",
                        "properties": {"invalid_count": {"type": "integer"}},
                    },
                },
            },
        },
...
KANBAN_COMPLETE_SCHEMA = {
    ...
    "description": (
        ...
        "downstream workers and humans. Put the exact, complete user-facing "
        "final answer in ``result``. Use ``summary`` for a separate "
        "human-readable 1-3 sentence handoff; put "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "role_shell_id": {
                "type": "string",
                "description": (
                    "Immutable active role-shell version id for supervisor-routed "
                    "work. Use this instead of assignee. The dispatcher selects a "
                    "real executor through the many-to-many binding registry and "
                    "records shell_id/executor_id/binding_id on every attempt."
                ),
            },
            "metadata": {
                "type": "object",
            ...
            "result": {
                "type": "string",
                "description": (
                    "Exact, complete user-facing final answer. This is the "
                    "canonical deliverable shown in Telegram, inspect, and "
                    "the Kanban dashboard; do not replace it with the short "
                    "summary or omit details that the user requested."
                ),
            },
            "created_cards": {
                "type": "array",
...
KANBAN_CAPABILITY_MISSING_SCHEMA = {
    "name": "kanban_capability_missing",
    "description": (
        "Escalate an absent MCP, skill, plugin, toolset, credential-backed "
        "capability, or browser surface to the Multitool role. The kernel "
        "creates an audited tool-management dependency card, parks the current "
        "card in todo, and automatically resumes it in a fresh worker session "
        "after the dependency completes. Do not use this for ordinary domain "
        "failure or for a capability that is already present."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "capability": {
                "type": "string",
                "description": (
                    "Missing capability in plain language; give an exact MCP, "
                    "skill, plugin, toolset, or access-surface name only when known."
                ),
            },
            "details": {
                "type": "string",
                "description": "Short failure evidence and why the current card requires it.",
            },
            "receipt": _receipt_schema_prop(),
            "board": _board_schema_prop(),
        },
...
registry.register(
    name="kanban_capability_missing",
    toolset="kanban",
    schema=KANBAN_CAPABILITY_MISSING_SCHEMA,
    handler=_handle_capability_missing,
    check_fn=_check_kanban_mode,
    emoji="🧰",
)
