...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        "coding_instructions": "",
        # Optional fail-closed language guard for user-visible assistant prose.
        # Empty/"auto" keeps the historical behavior.  "ko" validates the
        # final draft before persistence/delivery and asks the model to rewrite
        # mixed-script Korean up to a bounded retry limit.
        "response_language": "",
    ...
    "compression": {
        ...
        # The central supervisor is a long-lived control session whose small
        # tool contract matters more than retaining hundreds of verbatim turns.
        # Gateway hygiene applies this lower ceiling only to supervisor root;
        # compression remains in-place and preserves the session id.
        "supervisor_hygiene_hard_message_limit": 120,
    ...
    # Lightweight central supervisor. Disabled for ordinary upstream Hermes
    # installs; the public-edition setup enables it explicitly. In this
    # mode the gateway root never discovers/refetches MCP tools. Domain MCP and
    # tool surfaces belong only to isolated role-bound executors.
    "supervisor": {
        "enabled": False,
        "platforms": ["cli", "telegram", "discord", "slack"],
        # Public defaults contain no owner-specific services or schedules.
        # Operators declare these inventories explicitly after installation.
