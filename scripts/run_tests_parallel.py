...
_EXCLUSION_SCHEMA = "hermes.test-exclusions.v1"


def _test_nodeids_in_file(path: Path) -> set[str]:
    """Return statically declared pytest node suffixes in *path*."""
    ...
    class Visitor(ast.NodeVisitor):
        ...
        def visit_FunctionDef(self, node: ast.FunctionDef) -> None:  # noqa: N802
            if node.name.startswith("test_"):
                declared.add("::".join([*self.classes, node.name]))
...
def _apply_exclusion_manifest(
    manifest_path: Path,
    files: List[Path],
    repo_root: Path,
    *,
    validation_files: List[Path] | None = None,
) -> tuple[List[Path], List[str], dict[str, int]]:
    """Validate and apply an explicit, fail-closed exclusion manifest.

    ``validation_files`` can hold the repository-wide discovery set so stale
    exclusions are still detected when the current command runs one CI slice.
    Only exclusions that intersect ``files`` affect the current invocation.
    """
    try:
        data = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError(f"cannot read exclusion manifest {manifest_path}: {exc}") from exc

    if not isinstance(data, dict) or data.get("schema") != _EXCLUSION_SCHEMA:
        raise ValueError(f"exclusion manifest schema must be {_EXCLUSION_SCHEMA!r}")

    glob_entries = data.get("file_globs", [])
    node_entries = data.get("nodeids", [])
    if not isinstance(glob_entries, list) or not isinstance(node_entries, list):
        raise ValueError("file_globs and nodeids must be lists")

    candidates = validation_files if validation_files is not None else files
    candidate_by_rel = {_format_file(path, repo_root): path for path in candidates}
    selected_by_rel = {_format_file(path, repo_root): path for path in files}
    excluded_rel: set[str] = set()

    for entry in glob_entries:
        if not isinstance(entry, dict):
            raise ValueError("every file_globs entry must be an object")
        pattern = entry.get("pattern")
        reason = entry.get("reason")
        if not isinstance(pattern, str) or not pattern:
            raise ValueError("every file_globs entry requires a non-empty pattern")
        if not isinstance(reason, str) or not reason.strip():
            raise ValueError(f"file glob {pattern!r} requires a non-empty reason")
        matches = {
            rel for rel in candidate_by_rel if fnmatch.fnmatchcase(rel, pattern)
        }
        if not matches:
            raise ValueError(f"stale exclusion glob matched no test files: {pattern}")
        excluded_rel.update(matches & selected_by_rel.keys())

    deselect: list[str] = []
    for entry in node_entries:
        if not isinstance(entry, dict):
            raise ValueError("every nodeids entry must be an object")
        nodeid = entry.get("nodeid")
        reason = entry.get("reason")
        if not isinstance(nodeid, str) or "::" not in nodeid:
            raise ValueError("every nodeids entry requires an exact pytest nodeid")
        if not isinstance(reason, str) or not reason.strip():
            raise ValueError(f"nodeid {nodeid!r} requires a non-empty reason")

        rel, *suffix_parts = nodeid.split("::")
        path = candidate_by_rel.get(rel)
        if path is None:
            raise ValueError(f"stale exclusion nodeid file is not discoverable: {rel}")
        declared = "::".join(suffix_parts).split("[", 1)[0]
        if declared not in _test_nodeids_in_file(path):
            raise ValueError(f"stale exclusion nodeid is not declared: {nodeid}")
        if rel in selected_by_rel and rel not in excluded_rel:
            deselect.append(f"--deselect={nodeid}")

    filtered = [path for path in files if _format_file(path, repo_root) not in excluded_rel]
    stats = {
        "files_excluded": len(excluded_rel),
        "nodeids_deselected": len(deselect),
    }
    return filtered, deselect, stats
...
def _child_env_for_file(file: Path, repo_root: Path) -> dict[str, str]:
    """Build a subprocess environment with per-file Hermes state isolation.

    ``run_tests.sh`` may receive an explicit temporary ``HERMES_HOME`` to
    prevent tests from reading an operator's live configuration. Parallel
    files must not share that directory either: session databases, approval
    allowlists, and lock files are process-external state and can otherwise
    make unrelated tests race. Each file therefore gets a stable subdirectory
    under the supplied test home.
    """
    child_env = {**os.environ, "PYTHONDONTWRITEBYTECODE": "1"}
    hermes_home_base = os.environ.get("HERMES_HOME", "").strip()
    if not hermes_home_base:
        return child_env

    try:
        file_key = str(file.resolve().relative_to(repo_root.resolve()))
    except ValueError:
        file_key = str(file.resolve())
    digest = hashlib.sha256(file_key.encode("utf-8")).hexdigest()[:16]
    isolated_home = (
        Path(hermes_home_base).expanduser()
        / ".test-file-homes"
        / digest
    )
...
def _run_one_file(
    ...
    proc = subprocess.Popen(
        ...
        env=_child_env_for_file(file, repo_root),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Fail-closed JSON manifest of explicitly excluded file globs and "
            "exact pytest nodeids. Stale exclusions abort the run."
        ),
    )
    parser.add_argument(
        "paths_positional",
    ...
    if args.exclude_manifest:
        ...
        validation_files = _discover_files(
            [repo_root / root for root in _DEFAULT_ROOTS]
        )
        validation_files = sorted(set(validation_files) | set(files))
        try:
            files, deselect_args, exclusion_stats = _apply_exclusion_manifest(
                manifest_path,
                files,
                repo_root,
            ...
            return 2
        pytest_passthrough.extend(deselect_args)
        print(
            f"Exclusion manifest {_format_file(manifest_path, repo_root)}: "
            f"{exclusion_stats['files_excluded']} files excluded, "
            f"{exclusion_stats['nodeids_deselected']} nodeids deselected",
            flush=True,
        )
