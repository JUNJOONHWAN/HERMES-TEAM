...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        #
        # ``$BASHPID`` would be convenient here, but the Bash 3.2 shipped with
        # macOS does not define it.  ``mktemp`` provides a same-directory,
        # per-writer unique file on Linux, macOS, and Git Bash without relying
        # on shell-version-specific PID variables.
        _snap_template = shlex.quote(self._snapshot_path + ".tmp.XXXXXX")
        bootstrap = (
            f"__hermes_snap_tmp=$(command mktemp {_snap_template}) || exit 125\n"
            ...
            ">> \"$__hermes_snap_tmp\" 2>/dev/null || true\n"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # stock macOS Bash 3.2 has no BASHPID, so that suffix collapses to an
        # empty string and concurrent writers share one temp file.
        _snap_template = shlex.quote(self._snapshot_path + ".tmp.XXXXXX")
        ...
        if self._snapshot_ready:
            parts.append(
                f"if __hermes_snap_tmp=$(command mktemp {_snap_template}); then "
                "{ export -p > \"$__hermes_snap_tmp\" && "
                f"mv -f \"$__hermes_snap_tmp\" {_quoted_snap}; }} "
                "2>/dev/null || rm -f \"$__hermes_snap_tmp\" 2>/dev/null || true; fi"
            )
